...
while(<IN>)
{
	...
	if($num_line == 0)
	{
		...
		if( (exists $cut[$index{'PDF\_NMID'}] && $cut[$index{'PDF\_NMID'}] eq "\.\/\/")  or (exists $cut[$index{'pdf\_NMID'}] && $cut[$index{'pdf\_NMID'}] eq "\.\/\/") )
		{
			$exon="NULL";
		}else{
			$exon=join "\\\\", ( split /:/, $cut[$index{'PDF\_NMID'}] )[0,1] if(exists $cut[$index{'PDF\_NMID'}]);
			$exon=join "\\\\", ( split /:/, $cut[$index{'pdf\_NMID'}] )[0,1] if(exists $cut[$index{'pdf\_NMID'}]);
		}
		...
		if($cut[0]=~/^###/)
		{
			push @{ $site_dis_bz{$cut[7]}{$site_info} }, [ $cut[$index{'疾病名称'}], $cut[$index{'遗传模式'}], $acmg_info, $cut[$index{'致病性结论'}] ]  if(exists $index{'ACMG条目'});
			push @{ $site_dis_bz{$cut[7]}{$site_info} }, [ $cut[$index{'疾病名称'}], $cut[$index{'遗传模式'}], $acmg_info, $cut[$index{'位点结论'}] ]  if(exists $index{'ACMG对应条目'});
		}
		elsif($cut[0]=~/^##/)
		{
			$negative2=1;
			if(exists $index{'ACMG条目'})
			{
				...
				push @{ $site_dis_fb{$cut[7]}{$site_info}{$acmg_info} }, [ $cut[$index{'疾病名称'}], $cut[$index{'遗传模式'}], $acmg_info, $cut[$index{'致病性结论'}] ];	
			}
			if(exists $index{'ACMG对应条目'})
			{
				...
				push @{ $site_dis_fb{$cut[7]}{$site_info}{$acmg_info} }, [ $cut[$index{'疾病名称'}], $cut[$index{'遗传模式'}], $acmg_info, $cut[$index{'位点结论'}] ];
			}
			...
			if(exists $index{'位点结论'})
			{
				...
				{
					push @{ $site_dis{$cut[7]}{$site_info}{ $cut[$index{'位点描述'}] } }, [ $cut[$index{'位点结论'}], $cut[$index{'疾病名称'}], $cut[$index{'遗传模式'}], $cut[$index{'疾病描述'}], $acmg_info];
			...
			elsif(exists $index{'致病性结论'})
			{
				...
				if(!exists $disease_site_level{$cut[7]}{$site_info}{$cut[$index{'疾病名称'}]})
				{
					push @{ $site_dis{$cut[7]}{$site_info}{ $cut[$index{'位点描述'}] } }, [ $cut[$index{'致病性结论'}], $cut[$index{'疾病名称'}], $cut[$index{'遗传模式'}], $cut[$index{'疾病描述'}],$acmg_info];
...
\begin{longtable}{| C{13mm} | C{16mm} | C{17mm} | C{17mm} | C{7mm} | C{7mm} | m{16mm}<{\centering} | m{12mm}<{\centering} | m{34mm}<{\centering} | }
\hline
	...
	foreach my $key2 (sort { $gene_site_level2{$key1}->{$a} <=> $gene_site_level2{$key1}->{$b} } keys %{$gene_site_level2{$key1}})
	{
		...
		foreach my $key3 (sort { $site_dis{$key1}{$key2}->{$a} <=> $site_dis{$key1}{$key2}->{$b} } keys %{ $site_dis{$key1}{$key2} })
		{
			...
			for(my $i=0;$i<@arr;$i++)
			{
				...
				$c7   = $arr[$i][0];
				$c7=~s/临床意义未明/临床意义\\\\未明/g;

				if($arr[$i][2] eq "-" && $arr[$i][1] ne "-")
				{
					$arr[$i][2]="不明";
				}
				$c8="$arr[$i][1]($arr[$i][2])"             if($part2_flag==0);
				$c8="$arr[$i][1]($arr[$i][2],$arr[$i][0])" if($part2_flag==1);
				$c9=$arr[$i][4];
				
				if($j==0 )
				{
					$part2_2=qq[ \\multirow{$j_lv1_num}{*}{$c1} & \\multirow{$j_lv1_num}{*}{\\makecell[c]{$c22}} & \\multirow{$j_lv1_num}{*}{\\makecell[c]{$c33}} & \\multirow{$j_lv1_num}{*}{\\makecell[c]{$c4} } & \\multirow{$j_lv1_num}{*}{$c5} & \\multirow{$j_lv1_num}{*}{ \\makecell[{}{m{5.8mm}}]{$c6} } ]  if($j_lv1_num>1);
					$part2_2=qq[ $c1 & $c22 & $c33 & \\makecell[c]{$c4} & $c5 & \\makecell[{}{m{5.8mm}}]{$c6} ] if($j_lv1_num==1);
				}
				elsif($j>0 )
				{
					$part2_2=qq[ & & & & & ];
				...
				if($i==0)
				{
					$part2_2.=qq[ & \\multirow{$j_lv2_num}{*}{ \\makecell[{}{m{16mm}}]{$c9}} & \\multirow{$j_lv2_num}{*}{ \\makecell[{}{m{12mm}}]{$c7} } & ] if($j_lv2_num>1);
					$part2_2.=qq[ & \\makecell[{}{m{16mm}}]{$c9} & \\makecell[{}{m{12mm}}]{$c7} & ] if($j_lv2_num==1);
				}
				elsif($i>0 )
				{
					$part2_2.=qq[ & & & ];
				...
				if( ($j+1)==$j_lv1_num )
				{
					$part2_2.=qq[ \\makecell[{}{m{34mm}}]{$c8} \\\\ \\hline\n ];
				}
				elsif( ($i+1) < $j_lv2_num )
				{
					$part2_2.=qq[ \\makecell[{}{m{34mm}}]{$c8} \\\\ \\cline{9-9}\n ];
				}
				elsif( ($i+1) == $j_lv2_num && ($j+1)<$j_lv1_num )
				{
					$part2_2.=qq[ \\makecell[{}{m{34mm}}]{$c8} \\\\ \\cline{7-9}\n ];
				}
				$part2.=$part2_2;
...
#		$part4.=$s1."{\\sym{基因描述：}} ".$s2."\\\\ \\\\ \n{\\sym{位点描述：}} \\\\ ".$s3;
		if($j_lv1_num==1){
			...
			$part2.=qq[ \\multirow{$j_lv1_num}{*}{$c1} & \\multirow{$j_lv1_num}{*}{\\makecell[c]{$c22}} & \\multirow{$j_lv1_num}{*}{\\makecell[c]{$c33}} & \\multirow{$j_lv1_num}{*}{\\makecell[c]{$c4} } & \\multirow{$j_lv1_num}{*}{$c5} & \\multirow{$j_lv1_num}{*}{ \\makecell[{}{m{6.8mm}}]{$c6} } ].$part2_2;
		}
=cut
		...
		foreach my $key2 (sort {$gene_site_level_fb{$key1}->{$a} <=> $gene_site_level_fb{$key1}->{$b}} keys %{$gene_site_level_fb{$key1}})
		{
			...
			foreach my $key3 (sort { $site_dis_fb{$key1}{$key2}->{$a} <=> $site_dis_fb{$key1}{$key2}->{$b} } keys %{ $site_dis_fb{$key1}{$key2} })
			{
				...
				for(my $i=0;$i<@arr;$i++)
				{
					...
					$c7   = $arr[$i][3];
					#$c7=~s/临床意义未明/临床意义未明\\\\/g;

					if($arr[$i][1] eq "-" && $arr[$i][0] ne "-")
					{
						$arr[$i][1]="不明";
					}
					$c8="$arr[$i][0]($arr[$i][1])"             if($part2_flag==0);
					$c8="$arr[$i][0]($arr[$i][1],$arr[$i][3])" if($part2_flag==1);
					$c9=$key3; ###$arr[$i][4];			

					if($j==0 )
					{
						$part2_2=qq[ \\multirow{$j_lv1_num}{*}{$c11} & \\multirow{$j_lv1_num}{*}{\\makecell*[c]{$c22}} & \\multirow{$j_lv1_num}{*}{\\makecell*[c]{$c33}} & \\multirow{$j_lv1_num}{*}{\\makecell*[c]{$c4} } & \\multirow{$j_lv1_num}{*}{$c5} & \\multirow{$j_lv1_num}{*}{ \\makecell[{}{m{5mm}}]{$c6} } ] if($j_lv1_num>1);
						$part2_2=qq[ $c11 & $c22 & $c33 & \\makecell*[c]{$c4} & $c5 & \\makecell[{}{m{5mm}}]{$c6} ] if($j_lv1_num==1);
					...
					if($i==0)
					{
						$part2_2.=qq[ & \\multirow{$j_lv2_num}{*}{ \\makecell[{}{m{14mm}}]{$c9} } & \\multirow{$j_lv2_num}{*}{ \\makecell[{}{m{10.5mm}}]{$c7} } & ] if($j_lv2_num>1);
						$part2_2.=qq[ & \\makecell[{}{m{14mm}}]{$c9} & \\makecell[{}{m{10.5mm}}]{$c7} & ] if($j_lv2_num==1);
						#$part2_2.=qq[ & \\multirow{$j_lv2_num}{*}{ \\makecell[{}{m{13.8mm}}]{$c9} } & \\multirow{$j_lv2_num}{*}{ \\makecell[{}{m{13.8mm}}]{$c7} } & ] if($j_lv2_num>1);
						#$part2_2.=qq[ & \\makecell[{}{m{13.8mm}}]{$c9} & \\makecell[{}{m{13.8mm}}]{$c7} & ] if($j_lv2_num==1);
					}
					elsif($i>0 )
					{
						$part2_2.=qq[ & & & ];
					...
					{
						$part2_2.=qq[ $c8 \\\\ \\cline{9-9}\n ];
						#$part2_2.=qq[ \\makecell[{}{m{34mm}}]{$c8} \\\\ \\cline{9-9}\n ];
					}
					elsif( ($i+1) == $j_lv2_num && ($j+1)<$j_lv1_num )
					{
						$part2_2.=qq[ $c8 \\\\ \\cline{7-9}\n ];
						#$part2_2.=qq[ \\makecell[{}{m{34mm}}]{$c8} \\\\ \\cline{7-9}\n ];
					}
					$fb1.=$part2_2;
					$j++;					
...
%\begin{longtable}{| C{13mm} | C{15mm} | C{15mm} | C{15mm} | C{6mm} | C{6mm} | m{10mm}<{\centering} | C{16mm} | m{34mm} | } 
\begin{longtable}{| C{13mm} | C{16mm} | C{17mm} | C{17mm} | C{7mm} | C{7mm} | m{14mm}<{\centering} | m{12mm}<{\centering} | m{36mm}<{\centering} | }
%\begin{supertabular}{| m{13mm}<{\centering} | m{16mm}<{\centering} | m{17mm}<{\centering} | m{17mm}<{\centering} | m{7mm}<{\centering} | m{7mm}<{\centering} | m{14mm}<{\centering} | m{14mm}<{\centering} | m{34mm}<{\centering} | }
\hline
