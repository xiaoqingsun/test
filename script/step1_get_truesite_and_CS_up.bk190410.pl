...
sub CS_get_basesite{
	...
	while(<IN>){
		...
		while(<FILE>){
			...
			if( $tmp[52] ne '-' && $flag_OUT==0){ ### panqi 181017,181109
			...
			my @cut=&add_sitebase_info($re_info);
			for(@cut){
				my @arry=split /\t/,$_;
				...
				print OUT "$flag_2\t$prefix\t$re_info2\t-\t$_\t$tmp[12]\t$case_ditacion_ratio{$key22}\n";
			}
		}
		close FILE;
		$count++;
...
=cut
	...
	{ 
		...
		$add_info[10]=&pep_trans($cut[14]); #CDS    PEP
