	...
	while(<FI>){
		...
		while(<IN>){
			...
			for(my $k=0;$k<@all_site_dase_info;$k++)                 
			{
				...
				{
					push @filter_type, "non_pathogenic_DB" if($mu_type eq "nonsynonymous SNV");
				}#else{
				#	$hgmd_flag=1 if($mu_type eq "nonsynonymous SNV");
				#}
				...
				if( $mu_type eq "synonymous SNV" || $cut[5] eq 'intronic' || $cut[5]=~/UTR/ )
				{
					if( ($FLAG_DM_T62==1 && $FLAG_DM_62!=1) || ($FLAG_DM_T64==1 && $FLAG_DM_64!=1) )
					{
						...
						push @filter_type, "intronic_syn" if($cut[5] eq 'intronic');
						push @filter_type, "UTR" if($cut[5]=~/UTR/);
					}
				}
				...
				if ( ($cut[52] ne "-" and $flag!~/Man_pregnancy_hypertension/i ) or ($hgmd_flag==1 && $ratio1000>2 && $Disease_Namein_flag==0 && @filter_type<1) ){
					++$sample_number{$sample_id};
...
sub acmg_auto_judge{                    #wph add 190312       
	...
	if( ($cut[8]=~/^nonsynonymous/) && $cut[65] ne "-" ) 
	{
		...
		for(@site)
		{
			$flag_truetype=1 if( (split /:/,$_)[0] eq "true_type" );
			next if( (split /:/,$_)[0] ne "true_site" );
			$pep2=(split /:/,$_)[3];
