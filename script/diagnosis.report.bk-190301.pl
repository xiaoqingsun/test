	...
	foreach my $key2 (sort { $gene_site_level2{$key1}->{$a} <=> $gene_site_level2{$key1}->{$b} } keys %{$gene_site_level2{$key1}})
	{
		...
		foreach my $key3 (sort { $site_dis{$key1}{$key2}->{$a} <=> $site_dis{$key1}{$key2}->{$b} } keys %{ $site_dis{$key1}{$key2} })
		{
			...
			for(my $i=0;$i<@arr;$i++)
			{
				...
				if($i>0 && $c7_2 ne $arr[$i][0]){ $part2_flag=1; }
				$c7   = $arr[$i][0];
				$c7_2 = $arr[$i][0];
				#$c7   =~s/临床意义/临床意义\\\\/g if($c7=~/临床意义/);
				if($arr[$i][2] eq "-")
				{
					$arr[$i][2]="不明";
				}
				$c8="$arr[$i][1]($arr[$i][2])" if($i==0);
				$c8_2="$arr[$i][1]($arr[$i][2])";
				$c8.="\\\\$arr[$i][1]($arr[$i][2])" if($i>0);
				#$c8=~s/，/，\\\\/g if($c8=~/，/);
...
{\\sym{疾病描述：}} \\\\
$arr[$i][1]：$arr[$i][3] \\\\ \\\\ ];
					#$s3=qq[$arr[$i][1]：$arr[$i][4] \\\\ \\\\ ] if(@arr>1);
					$s3=qq[$key3 \\\\ \\\\ ];
				}
				elsif($i>0)
				{
					$s1.=qq[$arr[$i][1]：$arr[$i][3] \\\\ \\\\ ];
					#$s3.=qq[$arr[$i][1]：$arr[$i][4] \\\\ \\\\ ];
				}
				$part2_2.=qq[ $c1 & \\makecell[c]{$c2} & \\makecell[c]{$c3} & \\makecell[c]{$c4} & $c5 & $c6 & $c7 & \\makecell[{}{m{34mm}}]{$c8_2} \\\\ \\hline\n ];
			}
			
			$part2.=qq[ $c1 & \\makecell[c]{$c2} & \\makecell[c]{$c3} & \\makecell[c]{$c4} & $c5 & $c6 & $c7 & \\makecell[{}{m{34mm}}]{$c8} \\\\ \\hline\n ] if($part2_flag==0);
			$part2.=$part2_2 if($part2_flag==1);
			print $part2;
