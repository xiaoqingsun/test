...
def prepare_allDic(files,disease_file,filesfu):
    ...
    gene_li= []
    key_li=[]
    het_hom ={'het':u'杂合','hom':u'纯合','unknown':u'未明'}
        ...
        for line in files:
            ...
            if  key not in key_li:
                ...
                if data_Dic.has_key(key):
                    ...
                    sys.stderr.write(u'请检查filter内%s与GXY.disease.table中的gene名、疾病亚型名是否一致'%gene_dis_key2)
                    sys.exit(1)
            else:
                pass
            ##获取site_Dic
...
def get_zongshu(site_Dic,disease_Dic):
    ...
    for key in sorted(sortDic,key=lambda x:sortDic[x][0][11]):   ### panqi ???
        for n in sortDic[key]:
            ...
            het_hom.append(n[4])
    zongshu='此次单基因高血压/血钾异常基因检测，包含41个疾病相关基因，筛查15种疾病亚型。'
...
'AD，Smu':'常染色体显性遗传'}
    if site_num == 1:
        if mode_Dic.has_key(genetic_model[0]):
            ...
            sys.stderr.write(u'遗传模式为%s，不在AD AR XLD XLR XL AD/AR AR/AD  AD,AR  AR,AD  AD，AR  AR，AD内'%mode)
            sys.exit(1)
...
'''%(key,sortDic[key][0][0],sortDic[key][0][1],sortDic[key][0][2],sortDic[key][0][3],sortDic[key][0][4],sortDic[key][0][5],sortDic[key][0][6],sortDic[key][0][7],sortDic[key][0][8])
            ...
            for line in sortDic[key]:
                ...
                if  m==1:
                    table += r'''
\color{MyFontGray}
\multirow{%s}{*}{%s}  & %s & %s &  \makecell*[c]{%s} & %s & %s & %s & %s & %s  & %s  \\  \cline{2-10}
'''%(str(num),key,line[0],line[1],line[2],line[3],line[4],line[5],line[6],line[7],line[8])
                elif m==num:
                    table += r'''
\color{MyFontGray}
  & %s & %s &  \makecell*[c]{%s} & %s & %s & %s & %s & %s  & %s  \\  \hline
'''%(line[0],line[1],line[2],line[3],line[4],line[5],line[6],line[7],line[8])
                else:
                    table += r'''
\color{MyFontGray}
  & %s & %s &  \makecell*[c]{%s} & %s & %s & %s & %s & %s  & %s  \\  \cline{2-10}
'''%(line[0],line[1],line[2],line[3],line[4],line[5],line[6],line[7],line[8])
...
\zihao{-6} \color{MyFontGray} 预测软件包括 SIFT、PolyPhen2、MutationTaster、M-CAP和REVEL, 用于预测错义变异是否会导致蛋白结构和功能发生改变, 准确率大致在 65\%-80\%。需要说明的是, 各软件提供的只是基于算法的预测结果, 可以为位点解读提供参考, 但不足以作为致病性判定的唯一标准, 还需要结合其他信息综合判断。\\ \\
...
位点致病性: & %s \\
\end{tabular}}
\end{spacing} '''%(key,site_li[0],type_sub,level[0])
