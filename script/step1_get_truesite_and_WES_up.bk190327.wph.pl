...
sub CS_get_basesite{
	...
	while(<IN>){
		...
		while(<FILE>){
			...
			if( $tmp[52] ne '-' && $flag_OUT==0){ ### panqi 181017
			...
			if ($exits_flag){print OUT "$flag_2\t$prefix\t$re_info\t$cut22\t$tmp[12]\t$case_ditacion_ratio{$key22}\n";}
		}
		close FILE;
		$count++;
	}
...
=cut
	...
	$add_info[11]=&pep_trans($cut[14]); #CDS    PEP
	...
	$muty=~s/^-//g;
    $add_info[5]="本筛查检测出$cut[6]基因的变异，变异位点为$cut[13]，查询ClinVar等公共数据库显示，$cut[13]变异${muty}。$ratio_detection"."${software_pred}${splicingds}${wenxianzhichi}行ACMG标准，判定该变异的致病性为临床意义未明。具体情况请结合临床相关资料综合判断。";#位点描述
	#$add_info[6]="-"; #参考文献
