	...
	while(<FI>){
		...
		while(<IN>){
			...
			for(my $k=0;$k<@all_site_dase_info;$k++)                 
			{
				...
				my $site_dase_info_8=(split "\t", $all_site_dase_info[$k])[8];
				my @final_out=(
				...
				my @final_out2=split /\t/, $all_output;
				if($final_out2[70]=~/^chr/ && exists $gene_disease_relation{$site_dase_info_8}{$site_dase_info_6} ) ###add panqi 190709
				{
					$final_out2[70]=$gene_disease_relation{$site_dase_info_8}{$site_dase_info_6};
				}
				$all_output=join"\t",@final_out2;
...
sub add_sitebase_info{
	...
	if($cut[0]==5 && $cut[1]==136969729)
	{
        ...
        my  $ncon=0;
        if($cut[20] eq "."){$ratio_chinese='未收录'; $ncon++; push @detection_ratio_n ,'千人项目';}else{$ratio_chinese=$cut[20];}
        if($cut[21] eq "."){$ratio_ExAC='未收录'; $ncon++;  push @detection_ratio_n, 'ExAC数据库全球人群';}else{$ratio_ExAC=$cut[21];}
        if($cut[19] eq "."){$ratio_gnomAD = '未收录'; $ncon++; push @detection_ratio_n, 'gnomAD数据库全球人群';}else{$ratio_gnomAD=$cut[19];}
        if($cut[22] eq "."){$ratio_novo='未收录';$ncon++; push @detection_ratio_n,'本地数据库';}else{$ratio_novo=$cut[22];}
        my $ratio_detection='该变异在';
        if($ncon==4) {
               $ratio_detection.='千人项目人群、ExAC数据库全球人群、gnomAD数据库全球人群、本地数据库均未收录。';
        }elsif($ncon==0){
               $ratio_detection.="千人项目人群检出频率为$ratio_chinese，ExAC数据库全球人群检出频率为$ratio_ExAC，gnomAD数据库全球人群检出频率为$ratio_gnomAD，本地数据库检出频率为$ratio_novo。";
        }else{
              if($ratio_chinese ne '未收录'){$ratio_detection.="千人项目人群检出频率为$ratio_chinese，";}
              if($ratio_ExAC ne '未收录'){$ratio_detection.="ExAC数据库全球人群检出频率为$ratio_ExAC，";}
              if($ratio_gnomAD ne '未收录'){$ratio_detection.="gnomAD数据库全球人群检出频率为$ratio_gnomAD，";}
