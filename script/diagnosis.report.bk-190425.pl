...
foreach my $key1 (sort { $gene_level2{$a} <=> $gene_level2{$b} } keys %gene_level2)
{
	foreach my $key2 (sort { $gene_site_level2{$key1}->{$a} <=> $gene_site_level2{$key1}->{$b} } keys %{$gene_site_level2{$key1}})
	{
		...
		$s3='';
		my ($s3_1,$s3_2)=('','');
		if($c2 =~/\_/ && $c2 =~/del/) {$c2=~s/del/\\\\del/g;}
		if($c2 =~/\_/ && $c3 =~/del/) {$c3=~s/del/\\\\del/g;}
		if($c2 =~/\_/ && $c2 =~/ins/) {$c2=~s/ins/\\\\ins/g;}
		if($c2 =~/\_/ && $c3 =~/ins/) {$c3=~s/ins/\\\\ins/g;}
		...
		foreach my $key3 (sort { $site_dis{$key1}{$key2}->{$a} <=> $site_dis{$key1}{$key2}->{$b} } keys %{ $site_dis{$key1}{$key2} })
		{
			...
			for(my $i=0;$i<@arr;$i++)
			{
				...
				if($j==0 && $i==0)
				{
					$gene_level3{$key1}{d}{$arr[$i][1]}=qq[$arr[$i][1]：$arr[$i][3] \\\\ \\\\ \n] if(!exists $gene_level3{$key1}{d}{$arr[$i][1]});
#					$s1=
#qq[{\\sym {位点$sn：}} \\\\
#{\\sym{疾病描述：}} \\\\
#$arr[$i][1]：$arr[$i][3] \\\\ \\\\ \n];
...
#		$part4.=$s1."{\\sym{基因描述：}} ".$s2."\\\\ \\\\ \n{\\sym{位点描述：}} \\\\ ".$s3;
		...
		if($j==1){
			$part2.=qq[ $c1 & $c2 & $c3 & \\makecell[c]{$c4} & $c5 & \\makecell[{}{m{6.8mm}}]{$c6} ].$part2_2;
		}else{
			$part2.=qq[ \\multirow{$j}{*}{$c1} & \\multirow{$j}{*}{$c2} & \\multirow{$j}{*}{$c3} & \\multirow{$j}{*}{\\makecell[c]{$c4} } & \\multirow{$j}{*}{$c5} & \\multirow{$j}{*}{ \\makecell[{}{m{6.8mm}}]{$c6} } ].$part2_2;
		}
...
{\\sym{疾病描述：}} \\\\ \n];
	...
	{
		$part4.=$gene_level3{$key1}{d}{$key4};
	}
	$part4.=$gene_level3{$key1}{g};
	foreach my $key4 ( keys %{$gene_level3{$key1}{s} } )
	{
		$part4.=$gene_level3{$key1}{s}{$key4};
	}
}
