...
my $n=0;
my %index;
my ($line1,$line2,$line3,$line4,$line5,$line_all)=('','','','','','');
my $qita_GXY='';
my $qita_CS ='';
my $qita_DM ='';
my $qita_OGTT ='';
...
while(<IN>)
{
	chomp;
	@cut=split /\t/, $_;
	($line1,$line2,$line3,$line4,$line5,$line_all)=('','','','','','');
	...
	if($n==0)
	{
		...
		{
			$line2.="目前服用药物: ".$cut[ $index{"目前服用几种药物"} ]."种, \\ \\ ";
		}
		if($cut[ $index{"CCB"} ] ne "-" )
		{
			$line2.="CCB,";
		...
		{
			$line3.="LDL-C水平: ".$cut[ $index{"LDL-C水平"} ]."; \\ \\ ";
		}
