...
while(<IN>)
{
	...
	@cut=split /\t/,$_;
	$level_site=0;
	my ($gene_info, $dis_name, $dis_info, $acmg_info);
	...
	if($num_line == 0)
	{
		...
		if($cut[$index{'基因描述'}]=~/\[auto\]/ || $cut[$index{'基因描述'}]=~/\[DB\]/ || $cut[$index{'基因描述'}]=~/\[check\]/ )
		{
			$gene_info=(split /\[auto\]/, $cut[$index{'基因描述'}])[0] if($cut[$index{'基因描述'}]=~/\[auto\]/);
			$gene_info=(split /\[DB\]/, $cut[$index{'基因描述'}])[0] if($cut[$index{'基因描述'}]=~/\[DB\]/);
		...
		if($cut[$index{'疾病名称'}]=~/\[auto\]/ || $cut[$index{'疾病名称'}]=~/\[DB\]/ || $cut[$index{'疾病名称'}]=~/\[check\]/ )
		{
			$dis_name=(split /\[auto\]/, $cut[$index{'疾病名称'}])[0] if($cut[$index{'疾病名称'}]=~/\[auto\]/);
			$dis_name=(split /\[DB\]/, $cut[$index{'疾病名称'}])[0] if($cut[$index{'疾病名称'}]=~/\[DB\]/);
		...
		if($cut[$index{'疾病描述'}]=~/\[auto\]/ || $cut[$index{'疾病描述'}]=~/\[DB\]/ || $cut[$index{'疾病描述'}]=~/\[check\]/ )
		{
			$dis_info=(split /\[auto\]/, $cut[$index{'疾病描述'}])[0] if($cut[$index{'疾病描述'}]=~/\[auto\]/);
			$dis_info=(split /\[DB\]/, $cut[$index{'疾病描述'}])[0] if($cut[$index{'疾病描述'}]=~/\[DB\]/);
		...
		if(exists $index{'ACMG条目'}){    $acmg_info=join(" ", (split /\+/,$cut[$index{'ACMG条目'}]) ); }
		if(exists $index{'ACMG对应条目'}){$acmg_info=join(" ", (split /\+/,$cut[$index{'ACMG对应条目'}]) ); }
		...
		elsif($cut[0]=~/^##/)
		{
			...
			if(exists $index{'ACMG条目'})
			{
				...
				push @{ $site_dis_fb{$cut[7]}{$site_info} }, [ $dis_name, $cut[$index{'遗传模式'}], $acmg_info, $cut[$index{'致病性结论'}] ];
			}
			if(exists $index{'ACMG对应条目'})
			{
				...
				push @{ $site_dis_fb{$cut[7]}{$site_info} }, [ $dis_name, $cut[$index{'遗传模式'}], $acmg_info, $cut[$index{'位点结论'}] ];
			}
			...
			if(exists $index{'位点结论'})
			{
				...
				{
					push @{ $site_dis{$cut[7]}{$site_info}{ $cut[$index{'位点描述'}] } }, [ $cut[$index{'位点结论'}], $dis_name, $cut[$index{'遗传模式'}], $dis_info, $acmg_info ];
			...
			elsif(exists $index{'致病性结论'})
			{
				...
				if(!exists $disease_site_level{$cut[7]}{$site_info}{$dis_name})
				{
					push @{ $site_dis{$cut[7]}{$site_info}{ $cut[$index{'位点描述'}] } }, [ $cut[$index{'致病性结论'}], $dis_name, $cut[$index{'遗传模式'}], $dis_info, $acmg_info ];
