...
#	$gene_cnv{$cut[0]}.="\\\\$omimnm($cut[4])[$omimtp]" if( exists $gene_cnv{$cut[0]});
	...
	if(!exists $gene_omim{$cut[0]}{$cut[1]}){
		$gene_omim{$cut[0]}{$cut[1]} = 1;
		if($cut[4] eq ""){$cut[4]="未知";}
		if(!exists $gene_cnv{$cut[0]}){
			$gene_cnv{$cut[0]} ="$omimnm($cut[4])";
		}else{
			$gene_cnv{$cut[0]}.="\\\\$omimnm($cut[4])";
		}
	}
...
while(<IN>)
{
	...
	if(exists $gene_cnv{$cut[5]}){
		$gene_cnv_dis=$gene_cnv{$cut[5]}; 
	}
	else{
		$gene_cnv_dis="-(-)"; 
	}
	$part10_2.=qq[ $cnv_loc & $cnv_copy & $cut[5] & \\makecell[{}{m{48mm}}]{$gene_cnv_dis} \\\\ \\hline \n];
}
close IN;
	...
	if($num_line == 0)
	{
		...
		elsif($cut[0]=~/^##/)
		{
			...
			if(exists $index{'ACMG条目'})
			{
				...
				$disease_fb{ $cut[$index{'基因名称'}] }{ $cut[$index{'疾病名称'}] } = "$cut[$index{'疾病名称'}] : $cut[$index{'疾病描述'}]";
			...
			if(exists $index{'ACMG对应条目'})
			{
				...
				$disease_fb{ $cut[$index{'HGNC基因名'}] }{ $cut[$index{'疾病名称'}] } = "$cut[$index{'疾病名称'}] : $cut[$index{'疾病描述'}]";
...
\fontsize{8}{9.6}\selectfont
%\begin{longtable}{| C{15mm} | C{16mm} | C{18mm} | C{17mm} | C{8mm} | C{8mm} | C{18mm} | L{35mm} | }
\begin{longtable}{| C{13mm} | C{16mm} | C{17mm} | C{17mm} | C{7mm} | C{7mm} | m{18mm}<{\centering} | m{34mm} | m{10mm} | }
\hline
{\sym{变异基因}} & {\sym{核苷酸变异}} & {\sym{氨基酸变异}} & \makecell[c]{ \sym{转录本}\\ \sym{外显子} } & {\sym{变异状态}} & {\sym{变异类型}} & {\sym{致病性}} & \makecell[c]{\sym{相关疾病(遗传模式)}} & \makecell[c]{\sye{ACMG} \\ \sym{条目}} \\ \hline
END
...
#$arr[$i][1]：$arr[$i][3] \\\\ \\\\ \n];
			...
			$part2_2=qq[ & $c7 & \\makecell[{}{m{34mm}}]{$c8} & \\makecell[{}{m{9.5mm}}]{$c9} \\\\ \\hline\n ] if($j==0 && $max_row==1);
			$part2_2.=qq[ & & & & & & $c7 & \\makecell[{}{m{34mm}}]{$c8} & \\makecell[{}{m{9.5mm}}]{$c9} \\\\ \\cline{7-9}\n ] if($j>0 && $j+1!=$max_row);
			$part2_2.=qq[ & & & & & & $c7 & \\makecell[{}{m{34mm}}]{$c8} & \\makecell[{}{m{9.5mm}}]{$c9} \\\\ \\hline\n ] if($j>0 && $j+1==$max_row);
...
#		$part4.=$s1."{\\sym{基因描述：}} ".$s2."\\\\ \\\\ \n{\\sym{位点描述：}} \\\\ ".$s3;
		...
		if($j==1){
			$part2.=qq[ $c1 & \\makecell[c]{$c22} & \\makecell[c]{$c33} & \\makecell[c]{$c4} & $c5 & \\makecell[{}{m{5.8mm}}]{$c6} ].$part2_2;
		}else{
			$part2.=qq[ \\multirow{$j}{*}{$c1} & \\multirow{$j}{*}{\\makecell[c]{$c22}} & \\multirow{$j}{*}{\\makecell[c]{$c33}} & \\multirow{$j}{*}{\\makecell[c]{$c4} } & \\multirow{$j}{*}{$c5} & \\multirow{$j}{*}{ \\makecell[{}{m{5.8mm}}]{$c6} } ].$part2_2;
		}
...
if($types eq "WES")
{
	...
	while(<IN>)
	{
		...
		#	$disease_name.=",$cut[1]" if($i>0 && $cut[1] ne "高血压");
			...
			{
				if(!exists $tmp_gene{$key}){$disease_gene_line.="$key; ";}
				$tmp_gene{$key}=1;
			}
			$i++;
		}
		...
		while(<IN>)
		{
			...
			if($cut[0] eq $samples )
			{
				...
				if(!exists $sample_uniq{$samples} || $sample_uniq{$samples}!=1)
				{
					...
					$part1_list.="\\makecell[{}{p{4cm}}]{根据表型得到相关基因} & \\makecell[{{p{11cm}}}]{$disease_gene_line} \\\\ \\hline \n";
