...
#		$software_pred.='该变异在蛋白功能预测软件'.$sssware."中未收录。";
		...
		if(!exists $right_old{$key}){	
			...
			for(my $k=0;$k<@tmp;$k++)
			{
				...
				$add_info[4]="本筛查检测出$cut[6]基因的变异，变异位点为$cut[13]，查询ClinVar等公共数据库显示，$cut[13]变异${muty}。$ratio_detection"."${software_pred}${splicing_pred}${wenxianzhichi}行ACMG标准，判定该变异的致病性为临床意义未明。具体情况请结合临床相关资料综合判断。";#位点描述
		...
		if(exists $right_old{$key}){                                     ### panqi 20181025
			...
			for(my $k=0;$k<@tmp;$k++)
			{
				...
				$add_info[4] = "本筛查检测出$cut[6]基因的变异，变异位点为$cut[13]，查询ClinVar等公共数据库显示，$cut[13]变异${muty}。${ratio_detection}"."${software_pred}${splicing_pred}${wenxianzhichi}行ACMG标准，判定该变异的致病性为临床意义未明。具体情况请结合临床相关资料综合判断。"."<-->$site_info_tmp"; #位点描述
			}
		}
