...
my $gxygene_disease_file="/PUBLIC/pipline/database/siteFilterDB/product/gxy_public_file/gene_deseases/191016_v7_gene_desease"; #wph 191017 update
my $dmgene_disease_file="/PUBLIC/pipline/script/Report/XKFW/V3_180824/database_bak/DM_disease_list"; #wph changed 200103 "/PUBLIC/work/sunxiaoqing/cs_report/DM_disease_list";
#########################
...
#	print "$sample_report_database,$uptime_sa\n$all_report_database,$uptime_saAll\n$readdatabase,$uptime\n$readtrace,$uptimetrace\n";
	...
	if($type eq 'GXY'){
		...
		%sample_data=readGXY_sample_report_data($sample_report_database,\%all_disease,$uptime_sa);
	        ...
	        &updata_read_base_fubiao(\%database,$gxy_sitedb_path_fubiao,84,'XK_gxybase_fubiao',\%sample_data,$uptime_sa,$readfubao,$readdatabase);
		...
		%database=&updata_read_base($readdatabase,$readfubao,$cusi_sitedb_path,85,'XK_cusbase',\%sample_data,$uptime_sa);
        	&updata_read_base_fubiao(\%database,$cusi_sitedb_path_fubiao,85,'XK_CSbase_fubiao',\%sample_data,$uptime_sa,$readfubao,$readdatabase);
		...
		&updata_all_report_database($all_report_database,$cusi_report_all_path,'CuS_reportAllbase',\%sample_data,$uptime_sa);
	        ...
	        &updata_read_base_fubiao(\%database,$dm_sitedb_path_fubiao,85,'XK_DMbase_fubiao',\%sample_data,$uptime_sa,$readfubao,$readdatabase);
		...
		&updata_read_base_fubiao(\%database,$wes_sitedb_path_fubiao,85,'XK_WESbase_fubiao',\%sample_data,$uptime_sa,$readfubao,$readdatabase);
...
sub updata_read_base_fubiao{
	my ($hash,$baspath,$discol,$basn,$sample_datas,$uptime_saa,$readfubaoo,$basefile)=@_;
	make_path("$baspath/$uptime_saa");
        my $basefilename="$baspath/$uptime_saa/$basn$uptime_saa.right.txt";
        open(OUT,">$basefilename")||die;
        my %hash4;
	...
	while(<IN>){
		...
		if(exists $$sample_datas{fubiao}{$key2})
		{ 
			if(exists $$sample_datas{fubiaosite}{$key}){
				print OUT "T1$$sample_datas{fubiaosite}{$key}\n";
			}else{
				print OUT "$_\n";
			}
			$hash4{$key}=1;
	...
	foreach my $key(keys %{$$sample_datas{fubiaosite}}){
		if (!exists $hash4{$key})
		{
			print OUT "Z$$sample_datas{fubiaosite}{$key}\n";
			$hash4{$key} = 1;
		}
	}
...
sub updata_read_base{
	...
	if($readfubaoo){
		...
		while(<IN>){
			...
			### 升级位点
			if($discol == 85 && !exists $$sample_datas{site}{$key} && exists $$sample_datas{site2}{$key2} )
			{
				print OUT join("\t",@tmp[0..89])."\n";
			}
			
			if($discol == 84 && !exists $$sample_datas{site}{$key} && exists $$sample_datas{site2}{$key2} )
			{
				print OUT join("\t",@tmp[0..119])."\n";
			}
		}
		close IN;
	}
	if($basefile){
		...
		while(<IN>){
			...
			my $key=join("_",@tmp[3,4,6,7,$discol]);
			my $key2=join("_",@tmp[3,4,6,7]);
			next if(exists $$sample_datas{fubiao}{$key2});   ### 降级的位点
			$hash2{$key}=1;
			$hash3{$key2}=1;
...
sub readGXY_sample_report_data{
	...
	while(<IN>){
		...
		my $key4=join("_",@tmp[3,4,6,7]);
			...
			$hash{site2}{$key4}{$tmp[84]}=1;  ### panqi 190418
                        $tmp[0]=$uptime_saa;
                        $hash{reportsite}{$keysam}=join("\t",@tmp);
                }elsif($tmp[0] =~/fubiao/i){ 
			$hash{fubiao}{$key4}.="$_\t$uptime_saa\n";
			$hash{fubiaosite}{$key}="$_\t$uptime_saa";  ### panqi 190106
			$hash{fubiao2}{$key4}{$tmp[84]}=1;  ### panqi 190418
		}
	}
	close IN;
	return %hash;
...
sub readCS_sample_report_data{
	...
	while(<IN>){
		...
		if($tmp[0] =~/report/i){
			...
			$hash{site2}{$key4}{$tmp[85]}=1;  ### panqi 190418
			$tmp[0]=$uptime_saa;
			$hash{reportsite}{$keysam}=join("\t",@tmp);
		}elsif($tmp[0] =~/fubiao/i){
			$hash{fubiao}{$key4}.="$_\t$uptime_saa\n";
			$hash{fubiaosite}{$key}="$_\t$uptime_saa"; ### panqi 190106
			$hash{fubiao2}{$key4}{$tmp[85]}=1;  ### panqi 190418      
		}
	}
	close IN;
        return %hash;
