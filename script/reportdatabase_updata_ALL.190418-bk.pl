...
sub updata_read_base_fubiao{
	...
	while(<IN>){
		chomp;
		my @tmp=split /\t/,$_;
		my $key2=join("_",@tmp[3,4,6,7,$discol]);
		my $key=join("_",@tmp[3,4,6,7]);

		if(exists $$hash{$key})
		{ 
			if(exists $$sample_datas{fubiaosite}{$key2}){
				print OUT "T1$$sample_datas{fubiaosite}{$key2}\n";
			}else{
				print OUT "$_\n";
			}
		}
...
sub updata_read_base{
	...
	if($basefile){
		...
		while(<IN>){
			...
			my $key=join("_",@tmp[3,4,6,7,$discol]);
			my $key2=join("_",@tmp[3,4,6,7]);
			next if(exists $$sample_datas{fubiao}{$key2});
			$hash2{$key}=1;
			$hash3{$key2}=1;
