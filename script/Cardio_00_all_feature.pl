...
#`xlscat -s '\t' -R 3- $file>$file2` unless(-e $file2);
...
my $n=0;
my %index;
my ($line1,$line2,$line2_drug,$line3,$line4,$line5,$line6,$line_all)=('','','','','','','','');
my $qita_GXY='';
my $qita_CS ='';
my $qita_DM ='';
my $qita_OGTT ='';
...
while(<IN>)
{
	chomp;
	@cut=split /\t/, $_;
	($line1,$line2,$line2_drug,$line3,$line4,$line5,$line6,$line_all)=('','','','','','','','');
	...
	if($n==0)
	{
		...
		if($version eq "v2" && exists $cut[ $index{"卧位-醛固酮（pg/ml）  高/低？（参考范围）"} ] && $cut[ $index{"卧位-醛固酮（pg/ml）  高/低？（参考范围）"} ] ne "-" )
		{
			$line2.="卧位-醛固酮（pg/ml）: ".&sub_float($cut[ $index{"卧位-醛固酮（pg/ml）  高/低？（参考范围）"} ])."; \\ \\ ";
		}
		if($version eq "v2" && exists $cut[ $index{"立位-醛固酮（pg/ml）  高/低？"} ] && $cut[ $index{"立位-醛固酮（pg/ml）  高/低？"} ] ne "-" )
		{
			$line2.="（立位-醛固酮pg/ml）: ".&sub_float($cut[ $index{"立位-醛固酮（pg/ml）  高/低？"} ])."; \\ \\ ";
		}
		...
		if($version eq "v2" && exists $cut[ $index{"血钠（mmol/L）（参考范围）"} ] && $cut[ $index{"血钠（mmol/L）（参考范围）"} ] ne "-" )
		{
			$line2.="血钠（mmol/L）: ".$cut[ $index{"血钠（mmol/L）（参考范围）"} ]."; \\ \\ ";
		}
		if($version eq "v2" && exists $cut[ $index{"血镁（mmol/l）（参考范围）"} ] && $cut[ $index{"血镁（mmol/l）（参考范围）"} ] ne "-" )
		{
			$line2.="血镁（mmol/l）: ".$cut[ $index{"血镁（mmol/l）（参考范围）"} ]."; \\ \\ ";
		}
		if($version eq "v2" && exists $cut[ $index{"血氯（mmol/l）（参考范围）"} ] && $cut[ $index{"血氯（mmol/l）（参考范围）"} ] ne "-" )
		{
			$line2.="血氯（mmol/l）: ".$cut[ $index{"血氯（mmol/l）（参考范围）"} ]."; \\ \\ ";
		}
		if($version eq "v2" && exists $cut[ $index{"血钙（mmol/l）（参考范围）"} ] && $cut[ $index{"血钙（mmol/l）（参考范围）"} ] ne "-" )
		{
			$line2.="血钙（mmol/l）: ".$cut[ $index{"血钙（mmol/l）（参考范围）"} ]."; \\ \\ ";
		}
		...
		{
			$line6 = $cut[ $index{"其他激素或生化指标：指标名称及水平"} ];
		}
		...
		#$line6="其他激素或生化指标: ".$cut[ $index{"其他激素或生化指标：指标名称及水平"} ]."; \\ \\ ";
		...
		if($line6 ne ''){$line6=~s/,$//;$line6=~s/, \\ \\ $//;$line6=~s/; \\ \\ $//; $line_all.=  "{\\sym{其他激素或生化指标:}} ".$line6." \\\\ ";}
		if($line5 ne ''){$line5=~s/,$//;$line5=~s/, \\ \\ $//;$line5=~s/; \\ \\ $//; $line_all.=  "{\\sym{其他病史:}} ".$line5." ";}
		$line_all=~s/\\\\ $//;
		$line_all=~s/\%/\\\%/g;
		$line_all=~s/\_/\\\_/g;
		$line_all=~s/。;/;/g;#wph add 190417
		$line_all=~s/，;/;/g;
        	$line_all=~s/,  \\ \\  \\\\ {\\sym/  \\ \\  \\\\ {\\sym/g;#wph add 190717
		$line_all=~s/陈泽佳肾病生信分析，//;
		$line_all=~s/陈泽佳肾病生信分析//;
		print $line_all."\n";
	}
	$n++;
