...
sub shuru{
    ...
    while(<IN2>){
		...
		if($cut[12] =~ /建议换药/){
			$level=$cut[12];
			$level=~s/建议换药/不建议使用/g;  ### panqi 180928
		}
...
$_ \\par ];
				...
				if($arr[1] =~ /不建议使用/){
			        	$color='red';
					$ym='建议换药';
					if($arr[1] =~ /\*17:0\/1/)   ### panqi 180928
					{
						$ym2="*17:0/1";
						$ym3="(由于您的*17也出现杂合变异，药物代谢情况请综合考虑。)";
					}
					if($arr[1] =~ /\*17:1\/1/)
					{
						$ym2="*17:1/1";
						$ym3="(由于您的*17也出现纯合变异，药物代谢情况请综合考虑。)";
					}	
			    	}
			    	elsif($arr[1] =~ /增加剂量/){
					$color='orange';
...
\\textcolor{DarkBlue}{\\sym{用药建议：}}$arr[3] $ym3\\\\
\\end{spacing}
\\arrayrulecolor{DarkBlue} \\hline
\\vspace*{1.1cm}
];
