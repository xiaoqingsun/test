...
foreach my $p(@all_sites){
	...
	$st=$st>0?$st:0;
	my $en=$p->[4]+200;
	print OUT "$p->[0]\t$p->[1]\t$p->[2]\t$st\t$en\t$p->[5]\t$p->[3]\n";
}
close OUT;

`perl  /PUBLIC/pipline/script/Report/XKFW/V2_171130/script/substr_fa.pl   /PUBLIC/pipline/database/ref/hg19_24chr/human_g1k_v37c_decoy.fasta  $path/01products/substr_fa_position  > $path/check/substr_fa_position.fa`;
