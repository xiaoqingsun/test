...
END
	...
	if($reportflag=~/南方/){
		if($module=~m/GXY/i){
			$script.="python3 $bin/Cardio_GXY_dox.py -module GXY -sample $ID -site $right  -disease /PUBLIC/pipline/script/Report/XKFW/V3_180824/database/GXY.disease.table.3.txt -out $outdir/02individual/$ID -yinyang $outdir/02individual/$ID/$ID.gxy.yinYangXing.txt -part 2 -qc $in_dir/Stat_QC.xls -cfg $outdir/01products/cfg.txt -feature $outdir/01products/feature.list \n";
			$script.="ln -s $outdir/02individual/$ID/南方医院-诺禾心康-$ID\_$name\_单基因高血压基因检测报告.docx $outdir/03result/南方医院-诺禾心康-$ID\_$name\_单基因高血压血钾异常基因检测报告.docx\n";	
		}
		if($module=~m/CS/i){
			$script.="python3 $bin/Cardio_CS_dox.py -module CS -sample $ID -dir $cusi -out $outdir/02individual/$ID -yinyang $outdir/02individual/$ID/$ID.cs.yinYangXing.txt -part 2 -qc $in_dir/Stat_QC.xls -cfg $outdir/01products/cfg.txt -feature $outdir/01products/feature.list \n";
	...
	{   # wangpenghui add 20180531  #190314 wph 删除其他模块报告，只留全外
		...
		if($reportflag=~/南方/){
			$script.="python3 $bin/Cardio_WES_dox.py -module IDT -sample $ID -dir $IDT -out $outdir/02individual/$ID -yinyang $outdir/02individual/$ID.idt.yinYangXing.txt -part 2 -qc $in_dir/Stat_QC.xls -cfg $outdir/01products/cfg.txt -feature $outdir/01products/feature.list -genelist $outdir/sample_disea_gene.txt\n";
			$script.="ln -s $outdir/02individual/$ID/南方医院-诺禾心康-$ID\_$name\_全外显子基因检测.docx $outdir/03result/南方医院-诺禾心康-$ID\_$name\_全外显子基因检测.docx\n";
		}
	}
