...
sub right_site{#add flag,RT2,add no filter gene site wph changed 2018.8.21
	...
	if(exists $database{$key} ){  ##zhouwei  如果是false位点的话，那么解读库中的HGMD id号要写成-，否则不会进行匹配。在right文件中就会被过滤掉
		for(@{$database{$key}}){
			...
			if($aaa[11] !~ /NM/){
				$aaa[11] = $tmp[12];
				my $lll=join("\t",@aaa);
				print RT2 "Done_addNM\t$prefix\t$all2\t-\t$lll\t$case_ditacion_ratio{$key22}\n"; #add　the ID column wph replace $_ by -
				if($flag == 1){
					if(exists $sub_sampleline{$key2}){print RT "Ti";}
				...
				print RT2 "Done\t$prefix\t$all2\t-\t$_\t$case_ditacion_ratio{$key22}\n"; #add　the ID column
				if($flag == 1){
					if(exists $sub_sampleline{$key2}){print RT "Ti"; }
		...
		for(@{$fubiaobasee{$key}}){
			...
			if(!exists $outhash{$key2}){
				...
				if($aaa[12] !~ /NM/){
					$aaa[12] = $tmp[12];
					my $lll=join("\t",@aaa);
					print RT2 "DoneFubiao_addNM\t$prefix\t$all2\t$lll\t$case_ditacion_ratio{$key22}\n"; #add　the ID column
					if($flag == 1){
						print RT "DoneFubiao_addNM\t$prefix\t$all2\t$lll\t$case_ditacion_ratio{$key22}\n" if($flag_RT==0);
						print OUT "$flag_wrong\t$prefix\t$all2\t$lll\t$case_ditacion_ratio{$key22}\n"  if($flag_RT==1);
					}
				}else{
					print RT2 "DoneFubiao\t$prefix\t$all2\t$_\t$case_ditacion_ratio{$key22}\n"; #add　the ID column
					if($flag == 1){
	...
	for(@tmp3){
		...
		if(!exists $outhash{$key2} ){#wph changed
			foreach my $k(@allg){
				if($tmp[70]=~/^chr/ && exists $gene_disease_relation{$k}{$tmp22[7]} ){  ###add panqi 190709
					$tmp[70]=$gene_disease_relation{$k}{$tmp22[7]};
				}				
			}
			my $all2=join("\t",@tmp);
			print RT2 "Todo\t$prefix\t$all2\t$_\t$tmp[12]\t$case_ditacion_ratio{$key22}\n";
			if($flag == 1){
