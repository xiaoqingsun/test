...
tpl=DocxTemplate('/PUBLIC/pipline/script/Report/XKFW/V3_180824/tempt/南方医院报告模板-心血管.docx')
...
def argInit():
    ...
    global filterFile, part,outDir,sampleid,module,yinyang_file,cfgFile,QCfile,feature_file
    ...
    try:
        for line in filesfu:
            ...
            if site not in site_li_fubiao:
                ...
                mute_type=get_mut_type(line_list[9])   
                level_value=get_level_value(line_list[78]) 
                site_detail=[line_list[86],line_list[87],exon,het_hom[line_list[31].lower()],mute_type,line_list[21],line_list[77],line_list[78],level_value,line_list[83],line_list[84],site2]
...
def sort_Dic(Dic):
    ...
    for key,value in Dic.items():
        for line in value:
            ##获取检出率数字用于排序
            if '%' in line[3]:
                num_jianchu=re.search(r'([\d\.]+)%',line[3]).group(1)
            elif '不明' in line[3]:
                num_jianchu='0'
            elif 'rare' in line[3]:
                num_jianchu='0.01'
...
def get_disease_table(Dic,siteDic,BZDic):
    ...
    for key in sorted(sorted_Dic,key=lambda x:sorted_Dic[x][0][10]):  ##根据致病性排序
        ...
        for line in Dic_new[key]:
                lin={'n1':line[1],'n2':line[2],'n3':line[3],'n4':line[4]}
...
def get_fubiao_table(Dic,siteDic,wenxian_fu_dict):
    cover_Dic={}
...
def get_jiexi_table(disease,site):
    ...
    for key in sorted(site_sortDic,key=lambda x:site_sortDic[x][0][10]):
                ...
                if len(site_des)==1:
                    ...
                    index=0
                    for sline in sorted(site_des,key=lambda x:site_des[x][0][2]):
                        dislev=[]
...
def get_sanger_Inf(siteDic):
    ...
    for key in sorted(sortDic,key=lambda x:sortDic[x][0][10]):
        for line in sortDic[key]:
            ...
            path1=re.sub('01products/02Cus/CS.pdf.txt','',path1)
...
def main():
	...
	fubiao_table = get_fubiao_table(diseaseDic_fu,siteDic_fu,wenxian_fu_dict)
	...
	context['typea_num']=disease_table['typea_num']
	context['typeb_num']=disease_table['typeb_num']
	...
	out_file=outDir + '/N'+ sampleid+'.遗传性心血管基因检测Panel报告.docx'
	tpl.save(out_file)
