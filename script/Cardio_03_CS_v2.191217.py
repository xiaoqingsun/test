...
def get_fubiao_table(Dic,siteDic,wenxian_fu_dict): #wph add 190325
    ...
    wenxian=''
    DicK=[]
    maxgene=sorted(sortDic.keys(),key=lambda x:len(x))[-1] #取最长基因
    for key in sorted(sortDic,key=lambda x:sortDic[x][0][8]):
        ...
        if len(Dic[key])==1:
            for line in sortDic[key]:
                ...
                table += r'''
\color{MyFontGray}
%s & %s & %s &   \makecell*[c]{%s} & %s & %s & %s & \makecell*[c]{%s} & %s & %s \\ \hline'''%(key,line[0],pep,line[2],line[3],line[4],line[6],line[7],Dic[key][0][1],Dic[key][0][2])
                print len(line[0]),line[2],Dic[key][0][1]
                maxcds=max(maxcds,len(line[0]))
                maxnm=max(maxnm,len(line[2].split("\\\\")[0]))
                maxdis=max(maxdis,len(Dic[key][0][1].decode("utf-8")))
                if Dic[key][0][1] in "致心律失常性右室心肌病 9型":
                    dis_flag=1 
        elif len(Dic[key])>1:
            num=len(Dic[key])
            for line in sortDic[key]:
...
\raisebox{-2mm}{\multirow{%s}{*}{%s}}  & \raisebox{-2mm}{\multirow{%s}{*}{%s}} & \raisebox{-2mm}{\multirow{%s}{*}{%s}} & \raisebox{-2mm}{\multirow{%s}{*}{\makecell*[c]{%s}}} & \raisebox{-2mm}{\multirow{%s}{*}{%s}} & \raisebox{-2mm}{\multirow{%s}{*}{%s}} & \raisebox{-2mm}{\multirow{%s}{*}{%s}} & \raisebox{-2mm}{\multirow{%s}{*}{\makecell*[c]{%s}}} &  %s & %s \\ \cline{9-10}'''%(str(num),key,str(num),line[0],str(num),pep,str(num),line[2],str(num),line[3],str(num),line[4],str(num),line[6],str(num),strintsnew,line2[1],line2[2])
                            ...
                            table += r'''
\color{MyFontGray}
\raisebox{-2mm}{\multirow{%s}{*}{%s}}  & \raisebox{-2mm}{\multirow{%s}{*}{%s}} & \raisebox{-2mm}{\multirow{%s}{*}{%s}} & \raisebox{-2mm}{\multirow{%s}{*}{\makecell*[c]{%s}}} & \raisebox{-2mm}{\multirow{%s}{*}{%s}} & \raisebox{-2mm}{\multirow{%s}{*}{\makecell*[c]{%s}}} & \raisebox{-2mm}{\multirow{%s}{*}{%s}} & \raisebox{-2mm}{\multirow{%s}{*}{\makecell*[c]{%s}}} &  %s & %s \\ \cline{9-10}'''%(str(num),key,str(num),line[0],str(num),pep,str(num),line[2],str(num),line[3],str(num),bianyitype,str(num),line[6],str(num),strintsnew,line2[1],line2[2])
                            maxcds=max(maxcds,len(line[0]))
                            maxnm=max(maxnm,len(line[2].split("\\\\")[0]))
                            maxdis=max(maxdis,len(line2[1].decode("utf-8")))
                            if line2[1] in "致心律失常性右室心肌病 9型":
...
&   &  &    &   &  & &  & %s & %s   \\ \cline{9-10}'''%(line2[1],line2[2])
                        if len(line2[1].decode("utf-8")) == 13:
                            dis_flag=1
                        maxdis=max(maxdis,len(line2[1].decode("utf-8")))
        DicK.append(Dic[key])
...
&   &  &    &   &  & &  & %s & %s & %s  \\ \cline{9-10}'''%(line[1],line[2])    
        DicK.append(Dic[key])
    head="\\begin{supertabular}{| m{9mm}<{\\centering} | m{15mm}<{\\centering} | m{15mm}<{\\centering} | m{13mm}<{\\centering} | m{7mm}<{\\centering} | m{10mm}<{\\centering} | m{10mm}<{\\centering} | m{13mm}<{\\centering}  |m{32mm}<{\\centering} |m{6mm}<{\\centering} | }\n"
    wenxian = get_wenxianfu(DicK)
