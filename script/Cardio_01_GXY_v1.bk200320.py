...
def get_zongshu(site_Dic,disease_Dic):
    ...
    for key in sorted(sortDic,key=lambda x:sortDic[x][0][11]):   ### panqi ???
        for n in sortDic[key]:
            ...
            het_hom.append(n[4])
    zongshu='此次单基因高血压/血钾异常基因检测，包含45个疾病相关基因，筛查15种疾病亚型。'
    ...
    try:
        ...
        if yinyang == '0':
            ...
            with open('/PUBLIC/pipline/script/Report/XKFW/V2_171130/tempt/Cardio_GXYyin_temple.tex','r') as file_handle:
                ...
                num=len(id_filterfu)
                if num==0:
                    cover_Dic['yin']=r'''此次单基因高血压/血钾异常基因检测，包含45个疾病相关基因，筛查15种疾病亚型。检测结果显示您并未发生明确致病变异的位点，因此，您的疾病症状并不是由这些已知基因的致病变异引起的，可能是目前研究尚未涉及的基因变异，也可能是由于其他因素，如不良生活习惯、环境、其他疾病等引起。建议您注意生活细节，积极改变不良生活方式，并配合医生治疗。 

以上检测结果仅基于当前的科学研究，随研究文献的更新，疾病-基因的对应关系可能会发生变化，因此我们的报告具有一定的时效性 。
'''             
                else:
                    cover_Dic['yin']=r'''此次单基因高血压/血钾异常基因检测，包含45个疾病相关基因，筛查15种疾病亚型。检测结果显示您并未发生高风险的位点变异；一些变异位点由于相关研究较少、ACMG证据条目不足，基于现有研究判断其致病性等级较低，但不排除导致疾病的可能性，这类位点的致病性等级及相关证据条目展示在附表中，供您和临床医生参考。
