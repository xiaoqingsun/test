...
#############################主脚本区域###########################################
my (%case_ditacion_ratio, %control_ditaction_ratio, %doctor_patient_info, %genelist, %gene_disease, %disease, %disease_ratio, %hash_base, %hash_base2 ,%hash_base_fubiao, %hash_base_fubiao2 ,%database_site ,%hash_base_2,%hash_del,%right_old, %sampleid_sex, %novo_1256_af, %hyper_last_af);
...
#true_positive位点：不做过滤，前提满足Man_pregnancy_hypertension;
	...
	while(<FI>){
		...
		while(<IN>){
			...
			if($cut[0] eq "7" and $cut[1]==150696324 )
			{
				$cut[1]=150696324;
			}
			...
			for(my $k=0;$k<@all_site_dase_info;$k++)                 
			{
				...
				($site_dase_info_6 eq '-') && (push @filter_type, "No_disease")&&($Disease_Namein_flag=1);
				($patient_sex_inform=~/男/ and $site_dase_info_6=~/妊娠高血压综合/) && (push @filter_type, 'Man_pregnancy_hypertension') ;
...
sub right_site{
	...
	if( exists $hash_base{$key} )
	{
		...
		for(my $k=0;$k<@tmp;$k++)
		{
			...
			if($cut[30]=~/het/ and $add_info[7]=~/AR/){
...
sub acmg_auto_judge{                    #wph add 190312       
	...
	my @site;
        ...
        if($cut[34]!~/deleterious|damaging|-/i){
                push @acmg,"BP4";
        }
	...
	if( ($cut[8]=~/^nonsynonymous/) && $cut[63] ne "-")
	{
		...
		for(@site)
		{
			next if( (split /:/,$_)[0] ne "true_site" );
			$pep1=(split /:/,$_)[4];
...
=cut
	...
	my $flag_PM2=0;	
	if( ($cut[19] eq "." || $cut[19] eq "0") && ($cut[21] eq "." || $cut[21] eq "0") && $cut[45]>=20 && $adar=~/AD/ )
	{
		$flag_PM2=1;
	}
	...
	{
		$flag_PM2=1 if( $adar=~/AD/ );
	}
	if($flag_PM2==1){push @acmg,"PM2";}
	
	my $ratio1=$cut[19];
	$ratio1=0 if($cut[19] eq ".");
	my $ratio2=$cut[21];
	$ratio2=0 if($cut[21] eq ".");
	my $ratio3=$cut[20];
	$ratio3=0 if($cut[20] eq ".");
	my $ratio_max = max($ratio1,$ratio2);	
	if(!exists $disease_ratio{$disease}){print "$disease\t$site_info\n";}
	if($ratio_max < sqrt($disease_ratio{$disease}) && $adar=~/AR/i)
	{
		push @acmg,"PM2";
	}

	my $ratio_min = min($ratio1,$ratio2,$ratio3);
	if($ratio_min > ( 1-sqrt(1 - $disease_ratio{$disease}) ) && $adar=~/AD/i )
	{
		push @acmg,"BS1";
	}

	if($ratio_min > sqrt($disease_ratio{$disease}) && $adar=~/AR/i )
	{
		push @acmg,"BS1";
	}
