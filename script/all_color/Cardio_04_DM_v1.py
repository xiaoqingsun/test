            ...
            ##获取disease_Dic
                ...
                if gene_dis_key not in disease_li:     #排除相同gene（不同位点）有多个相同疾病
                     ...
                     if data_Dic[gene_dis_key]:
                         ...
                         sys.stderr.write(u'请检查filter内%s与DM_disease_data中的gene名、疾病亚型名是否一致'%gene_dis_key)
                         sys.exit(1)
                else:
                    pass
            ##获取site_Dic
...
def read_CSdata():
    data='/PUBLIC/pipline/script/Report/XKFW/V3_180824/database/diabetes_info.txt'
    disease_Dic=defaultdict(list)
    ...
    try:
        with open(data, 'rU') as file_handle:
            for line in file_handle:
                ...
                if len(line_list) < 4:
                    sys.stderr.write(u'DM_disease_data 列数不足 ')
...
def main():
    ...
    if module.upper() == 'DM':
        ...
        cover_Dic['title']='Part'+part+' — 单基因糖尿病基因检测结果'
    out_DM=open(outDir+'/'+sampleid+'_Cardio_DM.tex','w')
    ...
    try:
        ...
        if yinyang == '0':
            ...
            with open('/PUBLIC/pipline/script/Report/XKFW/V3_180824/tempt/Cardio_DMyin_temple.tex','r') as file_handle:
                DM_temp = Template(file_handle.read())
                ...
                num=len(id_filterfu)
                if num==0:
                    cover_Dic['yin']=r'''此次单基因糖尿病基因检测，包含38个疾病相关基因，筛查27种疾病亚型。检测结果显示您并未发生明确致病变异的位点，因此，您的疾病症状并不是由这些已知基因的致病变异引起的，可能是目前研究尚未涉及的基因变异，也可能是由于其他因素，如不良生活习惯、环境、其他疾病等引起。建议您注意生活细节，积极改变不良生活方式，并配合医生治疗。 

以上检测结果仅基于当前的科学研究，随研究文献的更新，疾病-基因的对应关系可能会发生变化，因此我们的报告具有一定的时效性 。
'''
                else:
                    cover_Dic['yin']=r'''此次单基因糖尿病基因检测，包含38个疾病相关基因，筛查27种疾病亚型。检测结果显示您并未发生高风险的位点变异；一些变异位
...
'''
                ...
                report = DM_temp.safe_substitute(cover_Dic)
                print >> out_DM,report
        else:
            id_filter,id_filterfu,id_filterbz = get_filter(filterFile,sampleid)
            with open('/PUBLIC/pipline/script/Report/XKFW/V3_180824/tempt/Cardio_DMyang_temple.tex','r') as file_handle:
                DM_temp = Template(file_handle.read())
