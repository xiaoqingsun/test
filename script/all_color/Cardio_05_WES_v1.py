...
def argInit():
    ...
    parser.add_argument(
        '-part', help='2 / 3 / 4 / 5 ', required=True)
    parser.add_argument(
    ...
    try:
        with open(filterFile, 'rU') as file_handle:
			...
			line_1=re.sub('%','\%',line)
                        ...
                        filter_bz.append(line_2.strip())
                elif re.match('^##',line.strip()): 
                    newsam=re.sub('##','',line_list[0])
                    if  newsam==sampleid:
                        line_1=re.sub('%','\%',line)
                        line_2=re.sub('_','\_',line_1)
                        filter_fu.append(line_2.strip())
                else:
...
def prepare_allDic(files,filesfu,filesbz,fff):
    ...
    site_li = []
    disease_li= []
    het_hom ={'het':u'杂合','hom':u'纯合'}
    #data_Dic=read_CSdata() disease_Dic[disease_key]=[line_list[4],line_list[1],line_list[5],line_list[9],'-']  # 基因，疾病亚型，遗传模式，临床症状
    if fff==1:
        try:
            for line in files:
                line_list=[x.strip() for x in line.strip().split('\t')]
            ...
            ##获取disease_Dic
                ...
                if gene_dis_key not in disease_li:     #排除相同gene（不同位点）有多个相同疾病
                    ...
                    detail=[line_list[85],line_list[83],line_list[84],line_list[76],'-']
    ...
    try:
        for line in filesfu:
            ...
            if site not in site_li_fubiao:
                  ...
                  mute_type=get_mut_type(line_list[9])   
                  level_value=get_level_value(line_list[78]) 
                  site_detail=[line_list[86],line_list[87],exon,het_hom[line_list[31].lower()],mute_type,line_list[21],line_list[77],line_list[78],level_value,line_list[82]]
    ...
    try:
        with open(data, 'rU') as file_handle:
            for line in file_handle:
                ...
                if len(line_list) < 4:
                    sys.stderr.write(u'WES_disease_data 列数不足 ')
		    sys.stderr.write(line_2)
                    sys.exit(1)
                disease_key=re.sub(' ','',line_list[4])+':'+re.sub(' ','',line_list[1])       #gene:disease_subtype作为key
                disease_Dic[disease_key]=[line_list[4],line_list[1],line_list[5],line_list[9],'-']  # 基因，疾病亚型，遗传模式，临床症状
    except Exception as e:
        traceback.print_exc(e)
    return disease_Dic
...
def get_disease_table(Dic,siteDic,BZDic):
    ...
    for key in sorted(sorted_Dic,key=lambda x:sorted_Dic[x][0][10]):  ##根据致病性排序
        if len(Dic_new[key]) ==1:
            ADAR.append(Dic[key][0][2])
            table += r'''
%s & %s & %s & %s \\ \hline'''%(key,Dic_new[key][0][1],Dic_new[key][0][2],Dic_new[key][0][3])
        else:
            for li in Dic_new[key]:
                ADAR.append(li[3])
    ...
    cover_Dic['type_lite']=get_lite(litea)#,liteb)
...
def get_rows(gene,li):
    ...
    for line in li:
        ...
        if m==1:
            table += r'''
\multirow{%s}{*}{%s} & %s & %s & %s  \\ \cline{2-4}'''%(str(len(li)),gene,line[1],line[2],line[3])
        elif m==len(li):
            table += r'''
& %s & %s & %s & %s \\ \hline'''%(line[1],line[2],line[3])
        else:
            table +=r'''
& %s & %s & %s & %s \\ \cline{2-4}'''%(line[1],line[2],line[3])
    return table
...
def prepare_zhushi(Dic):
    ...
    for key,value in Dic.items():
        for line in value:
            ...
            if line[4] !='-' and line[4] !='' and line[4] !='.':literatureA.append(line[4])   ##获取A类文献
    ...
    for key in sorted(sortDic,key=lambda x:sortDic[x][0][8]):
        ...
        if len(Dic[key])==1:
            for line in sortDic[key]:
                table += r'''
\color{MyFontGray}
%s & %s & %s &   \makecell*[c]{%s} & %s & %s & %s & %s & %s & %s \\ \hline'''%(key,line[0],line[1],line[2],line[3],line[4],line[6],line[7],Dic[key][0][1],Dic[key][0][2])
        elif len(Dic[key])>1:
            num=len(Dic[key])
            for line in sortDic[key]:
                mm=0
                for line2 in Dic[key]:
                    ...
                    if mm==1:
                        strints=line[7]
                        strintsnew=re.sub('临床意义未','临床意义未\\\\\\\\',strints)
                        value=len(line[4])
                        print value
                        print line[4]
                        if value <= 12 :
...
下表为无ACMG证据、证据矛盾（偏致病类证据与偏良性证据共存）或偏良性证据的位点（可能与相关疾病出现的表型有关，具体情况请请结合临床综合分析）
\par \vspace*{6mm}
...
\tablefirsthead{
\rowcolor{DarkBlue}  
\color{white}{\sym{基因}} & \color{white}{\sym{ 核苷酸变异}} & \color{white}{\sym{氨基酸变异}}  &  \makecell[c]{\color{white} \sym{转录本}\\ \sym{外显子编号}}  &  \makecell[c]{\color{white}{\sym{变异}\\\sym{状态}}}  & \color{white}{\sym{变异类型}} &  \color{white}{\sym{ACMG条目}}  &  \color{white}{\sym{位点致病性}} & \color{white}{\sym{相关疾病}} & \makecell[c]{\color{white}{\sym{遗传} \\ \sym{模式} }} \\}
\tablehead{}
\tabletail{\hline}
\tablelasttail{}
\begin{supertabular}{| m{6mm}<{\centering} | m{15mm}<{\centering} | m{13mm}<{\centering} | m{15mm}<{\centering} | m{7mm}<{\centering} | m{10mm}<{\centering} | m{12mm}<{\centering} |m{14mm}<{\centering}| m{27mm}<{\centering} | m{6mm}<{\centering} | }
'''
...
\end{spacing}'''
    ...
    if biaoshi==1:
        ...
        fubiao_wenxiana=get_wenxianfu(DicK)
...
& %s  & %s &   \makecell*[c]{%s} &  %s & %s & %s  & %s  & &\\ \hline'''%(line[0],line[1],line[2],line[3],line[4],line[6],line[7])
                    ...
                    table += r'''
\color{MyFontGray}
& %s & %s &   \makecell*[c]{%s} &  %s & %s & %s  & %s & & \\ \cline{2-8}'''%(nue,acc,line[2],line[3],line[4],line[6],line[7])
        elif m1 == 1 and  m2 > 1:
            mm=0
            num=len(Dic[key])
            for line in Dic[key]:
...
\tablefirsthead{
\rowcolor{DarkBlue}  
\color{white}{\sym{基因}} & \color{white}{\sym{ 核苷酸变异}} & \color{white}{\sym{氨基酸变异}}  &  \makecell[c]{\color{white} \sym{转录本}\\ \sym{外显子编号}}  & \color{white}{\sym{变异状态}} & \color{white}{\sym{变异类型}} &  \color{white}{\sym{ACMG条目}}  &  \color{white}{\sym{位点致病性}} & \color{white}{\sym{相关疾病}} & \color{white}{\sym{遗传模式}} \\}
\tablehead{}
\tabletail{\hline}
\tablelasttail{}
\begin{supertabular}{| m{6mm}<{\centering} | m{15mm}<{\centering} | m{13mm}<{\centering} | m{15mm}<{\centering} | m{7mm}<{\centering} | m{10mm}<{\centering} | m{12mm}<{\centering} |m{14mm}<{\centering}| m{27mm}<{\centering} | m{6mm}<{\centering} | }
...
def main():
    ...
    if module.upper() == 'WES':
        ...
        cover_Dic['title']='Part'+part+' — 全外显子基因检测结果'
    out_WES=open(outDir+'/'+sampleid+'_Cardio_WES.tex','w')
    ...
    try:
        ...
        if yinyang == '0':
            ...
            with open('/PUBLIC/pipline/script/Report/XKFW/V3_180824/tempt/Cardio_IDTyin_temple.tex','r') as file_handle:
                WES_temp = Template(file_handle.read())
                ...
                num=len(id_filterfu)
                if num==0:
                    cover_Dic['yin']=r'''此次全外显子基因检测，检测结果显示您并未发生明确致病变异的位点，因此，您的疾病症状并不是由这些已知基因的致病变异引起的，可能是目前研究尚未涉及的基因变异，也可能是由于其他因素，如不良生活习惯、环境、其他疾病等引起。建议您注意生活细节，积极改变不良生活方式，并配合医生治疗。 

以上检测结果仅基于当前的科学研究，随研究文献的更新，疾病-基因的对应关系可能会发生变化，因此我们的报告具有一定的时效性 。
'''
                else:
                    cover_Dic['yin']=r'''此次全外显子基因检测，检测结果显示您并未发生高风险的位点变异；一些变异位点由于相关研究较少、ACMG证据条目不足，基于现有研究判断其致病性等级较低，但不排除导致疾病的可能性，这类位点的致病性等级及相关证据条目展示在附表中，供您和临床医生参考。
...
'''
                ...
                report = WES_temp.safe_substitute(cover_Dic)
                print >> out_WES,report
        else:
            id_filter,id_filterfu,id_filterbz = get_filter(filterFile,sampleid)
            with open('/PUBLIC/pipline/script/Report/XKFW/V3_180824/tempt/Cardio_IDTyang_temple.tex','r') as file_handle:
                WES_temp = Template(file_handle.read())
