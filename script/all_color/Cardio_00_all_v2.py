...
def argInit():
    ...
    parser.add_argument(
        '-dm', help='单基因糖尿病的阴阳性判定文件dm.yinYang.txt', required=True)
    parser.add_argument(
        '-wes', help='全外的阴阳性判定文件dm.yinYang.txt', required=True)
    parser.add_argument(
        '-wesgene', help='全外的检测的基因列表')
    parser.add_argument(
        '-module', help='CS+GXY+Drug or CS or GXY or ^', required=True)
    parser.add_argument(
        '-out', help='project name', default=os.getcwd())                   ##########缺失路径
    argv = vars(parser.parse_args())
    global QCfile, cfgFile, sampleid, Module,outDir,module,cs_file,gxy_file,dm_file,wes_file,wesgene_file
    ...
    dm_file = argv['dm'].strip()
    wes_file = argv['wes'].strip()
    wesgene_file=''
    if argv['wesgene'] is not None:
         wesgene_file = argv['wesgene'].strip() 
...
def get_Title_Letter(module,Module):
    title_file='/PUBLIC/pipline/script/Report/XKFW/V3_180824/database/cover.txt'
    ...
    if os.path.isfile(title_file) == False:
        ...
        exit
    try:
        partlen=len(module_li)
        if 'DM' in module_li or 'WES' in module_li:
            cover_Dic['Title']='基因检测报告'
            cover_Dic['Letter']=''
            cover_Dic['Part']=partlen+2
            cover_Dic['header']=''
            cover_Dic['Fuwuliucheng']='基因检测'
        else:
...
def get_sampleInf(sampleid,cfgFile):
    sample_inf_list=[]
    cover_Dic = defaultdict(list)
    head=['id','name','sex','age','date','chenghu']
        ...
        with open(cfgFile, 'rU') as file_handle:
            for line in file_handle:
                ...
                if line_list[0]==sampleid:
                    ...
                    sample_inf_list=[line_list[0],line_list[4],line_list[5],age,date,chenghu]  #需要送样日期，暂时用出报告日期代替
                    break
...
def get_HTB(sampleid):
    ...
    files=glob.glob(r'/PUBLIC/pipline/database/sheet_hutong/*心康送样信息单*.xls*')
    max_num=0
    age=date=''
    for i in files:
        hutong=i.split('/')[-1]
        #st=re.match(r'心康互通表（([0-9]*)）.xlsx',hutong)
        st=re.match(r'2018心康送样信息单-([0-9]*).xlsx',hutong)
        if int(st.group(1))> max_num:
            max_num=int(st.group(1))
    #HTB=r'/PUBLIC/pipline/database/sheet_hutong/心康互通表（%s）.xlsx' %max_num
    HTB=r'/PUBLIC/pipline/database/sheet_hutong/2018心康送样信息单-%s.xlsx' %max_num
    try:
		...
		if isinstance(age, float) == False:
			age_num=re.match(u'(\d+)岁',age).group(1)
		else:
			age_num=int(age)
			#age_num=re.match(u'(\d+)',age).group(1)
                date_num=re.sub(r'/',r'-',date)
                break
            else:
                pass
    ...
    try:
        if len(module_li) == 1:
            if module_li[0] == 'ALL':
                ...
                Jiance_li.append(get_DM('检测项目'))
            elif module_li[0] == 'WES':
                Jiance_li.append(get_WES('检测项目'))
            ...
            if 'ALL' in module_li:
                ...
                if 'DM' in module_li:
                    m += 1
                    num = '检测项目' + str(m)
                    Jiance_li.append(get_DM(num))
                if 'WES' in module_li:
                    m += 1
                    num = '检测项目' + str(m)
                    Jiance_li.append(get_WES(num))
            else:
                if 'GXY' in module_li:
                    m += 1
                    num = '检测项目' + str(m)
...
def get_GXY(part):
    ...
    GXY_file='/PUBLIC/pipline/script/Report/XKFW/V3_180824/database/gxy_disease_name2.txt'
    GXYxiang = r'''
\begin{center} \zihao{3}{\color{DarkBlue} \sye \sym %s} \end{center}
\par %%\vspace*{6mm}
\fontsize{10.5}{12.6}\selectfont{\color{MyFontGray} }本检测共筛查15种单基因高血压/血钾异常疾病，包含41个相关基因，具体疾病名称请参见下表。 \par'''%part
    GXYxiang += r'''
\begin{spacing}{1.2}
...
def get_DM(part):
    DM_Dic=defaultdict(list)
    DM_file='/PUBLIC/pipline/script/Report/XKFW/V3_180824/database/dm_disease_name.txt'
    DMxiang = r'''
\begin{center} \zihao{3}{\color{DarkBlue} \sye \sym %s} \end{center}
\par %%\vspace*{6mm}
\fontsize{10.5}{12.6}\selectfont{\color{MyFontGray} }本检测共筛查27种单基因糖尿病，包含38个相关基因，具体疾病名称请参见下表。 \par'''%part
    DMxiang += r'''
\begin{spacing}{1.2}
\renewcommand\arraystretch{2.1}
\color{MyFontGray} \zihao{-5}
\arrayrulecolor{DarkBlue}
\rowcolors{2}{}{LightBlue}
\begin{longtable}{L{8.5cm} L{7cm} }
\rowcolor{DarkBlue}
\zihao{-4}{\color{white} \sym{检测疾病}} & \zihao{-4}{\color{white} \sym{致病基因}} \\ '''
    if os.path.isfile(DM_file) == False:
        print('%s is not a file' % DM_file)
        return DMxiang
    try:
        with open(DM_file, 'rU') as file_handle:
            for line in file_handle:
                line_list=[x.strip() for x in line.strip().split('\t')]
                DMxiang += r'''
\makecell[{{p{8.2cm}}}]{%s} & \makecell[{{p{7cm}}}]{%s} \\ ''' %(line_list[0],line_list[1])
...
def get_WES(part):
    WES_Dic=defaultdict(list)
    WES_file=wesgene_file
    WESxiang = r'''
\begin{center} \zihao{3}{\color{DarkBlue} \sye \sym %s} \end{center}
\par %%\vspace*{6mm}
\fontsize{10.5}{12.6}\selectfont{\color{MyFontGray} }本检测相关疾病的相关基因，与其他单项重复出现的基因，不重复描述，具体疾病及基因列表请参见下表。 \par'''%part
    WESxiang += r'''
\begin{spacing}{1.2}
\renewcommand\arraystretch{2.1}
\color{MyFontGray} \zihao{-5}
...
\rowcolor{DarkBlue}
\zihao{-4}{\color{white} \sym{检测疾病}} & \zihao{-4}{\color{white} \sym{致病基因}} \\ '''
    if os.path.isfile(WES_file) == False:
        print('%s is not a file' % WES_file)
        return WESxiang
    try:
        with open(WES_file, 'rU') as file_handle:
            for line in file_handle:
                line_list=[x.strip() for x in line.strip().split('\t')]
                WESxiang += r'''
\makecell[{{p{8.2cm}}}]{%s} & \makecell[{{p{7cm}}}]{%s} \\ ''' %(line_list[1],line_list[2])
        WESxiang += r'''
\hline
\end{longtable}
\end{spacing}'''
...
\begin{center} \zihao{3}{\color{DarkBlue} \sye \sym %s} \end{center}
\par %%\vspace*{6mm}
\fontsize{10.5}{12.6}\selectfont{\color{MyFontGray} 本次心血管用药指导基因检测包括45个基因，共涉及13大类、40种心血管常用药，具体用药名称请参照下表。 \par'''%part
...
def get_insert_module(module,sampleid,gxy,cs,dm,wes):
    CS_Dic={}
    insert=''
    insert2=''
    include_gxy=r'\include{%s_Cardio_GXY}'%sampleid
    include_cs=r'\include{%s_Cardio_CS}'%sampleid
    include_dm=r'\include{%s_Cardio_DM}'%sampleid
    include_wes=r'\include{%s_Cardio_WES}'%sampleid
...
\end{spacing}'''
    ...
    module_new=[]
    if 'ALL' in module_li:
    	module_new=['GXY','DRUG','CS']
        if 'DM' in module_li:
            module_new.append('DM')
        if 'WES' in module_li:
            module_new.append('WES')
    else:
	module_new=module_li
    print module_new
    insert2_flag=0
    if 'GXY' in module_new:
        insert += include_gxy
        if gxy=='1':
            insert2_flag+=1
    if 'CS' in module_new:
    ...
    if 'DM' in module_new:
        ...
        if dm =='1':
            insert2_flag+=1        
    if 'WES' in module_new:
        insert += include_wes
        if wes =='1':
            insert2_flag+=1
    if insert2_flag>0:
        insert += mingcishuoming
        insert2 = include_sanger
    if 'DRUG' in module_new:
        insert += include_drug
...
def main():
    ...
    if os.path.isfile(gxy_file) == False:
        ...
        sys.exit(1)
    if os.path.isfile(dm_file) == False:
        sys.stderr.write('%s is not a file' %dm_file)
        sys.exit(1)
    if os.path.isfile(wes_file) == False:
        sys.stderr.write('%s is not a file' %wes_file)
        sys.exit(1)
    ...
    gxy_handle.close()
    dm_handle = open(dm_file,'r')
    dm_yinyang = dm_handle.readlines()[0].strip()
    dm_handle.close()
    wes_handle = open(wes_file,'r')
    wes_yinyang = wes_handle.readlines()[0].strip()
    wes_handle.close()
    cover_Dic = defaultdict(list)
    cover_Dic = get_Title_Letter(module,Module)
    ...
    insert_Dic = get_insert_module(module,sampleid,gxy_yinyang,cs_yinyang,dm_yinyang,wes_yinyang)
    ...
    with open('/PUBLIC/pipline/script/Report/XKFW/V3_180824/tempt/Cardio_all_temple.tex','r') as file_handle:
