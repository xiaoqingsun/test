...
my ($samples,$site_file,$out,$types,$qc_stat,$hospital,$usage,$genelist,$feature_file,$cnv_file,$gene_disease_file)=('','','','','','','','','','','');
GetOptions(
    'sample=s'=> \$samples,
    'hospital=s'=> \$hospital,
    'site=s' => \$site_file,
...
while(<IN>)
{
	...
	$gene_cnv{$cut[0]} ="$cut[1]($cut[2])"     if(!exists $gene_cnv{$cut[0]});
	$gene_cnv{$cut[0]}.="\\\\$cut[1]($cut[2])" if( exists $gene_cnv{$cut[0]});
}
close IN;
...
while(<IN>)
{
	...
	$part10_2.=qq[ $cnv_loc & $cnv_copy & $cut[5] & \\makecell[{}{m{34mm}}]{$gene_cnv_dis} \\\\ \\hline \n];
}
close IN;
