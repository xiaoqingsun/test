...
my ($yemei,$biaoti,$biaoti2,$info_sample,$info1,$part1,$part2,$part3,$part4,$part5,$part6,$part6_0,$part7_1,$part7,$part8,$check,$BZ3);
...
###########################
		...
		elsif($cut[0]=~/^##/)
		{
			...
			{
				$gene_site_level_fb{$cut[7]}{$site_info}{$acmg_info}=$level_value{ $cut[$index{'致病性结论'}] };
			...
			{
				$gene_site_level_fb{$cut[7]}{$site_info}{$acmg_info}=$level_value{$cut[$index{'位点结论'}]};
...
if(%site_dis_fb)
{
	...
	{
		foreach my $key2 (sort {$gene_site_level_fb{$key1}->{$a} cmp $gene_site_level_fb{$key1}->{$b}} keys %{$gene_site_level_fb{$key1}})
		{
			...
			my $part2_2='';
			my $part2_flag=0;
			foreach my $key3 (sort { $gene_site_level_fb{$key1}{$key2}->{$a} <=> $gene_site_level_fb{$key1}{$key2}->{$b} } keys %{ $gene_site_level_fb{$key1}{$key2} })
			{# $cut[$index{'疾病名称'}], $cut[$index{'遗传模式'}], $acmg_info, $cut[$index{'位点结论'}]
				$part2_flag=0;
			...
			if($j==1){
				$fb1.=qq[ $arr[0] & $arr[1] & $arr[2] & \\makecell[c]{$arr[3]} & $arr[4] & $arr[5] ].$part2_2;
			}else{
				$fb1.=qq[ \\multirow{$j}{*}{$arr[0]} & \\multirow{$j}{*}{$arr[1]} & \\multirow{$j}{*}{$arr[2]} & \\multirow{$j}{*}{\\makecell[c]{$arr[3]} } & \\multirow{$j}{*}{$arr[4]} & \\multirow{$j}{*}{ \\makecell[{}{m{5.8mm}}]{$arr[5]} } ].$part2_2;
			}
		}
	}
...
END
	...
	if($types eq "GXY")
	{
		$part1.="此次单基因高血压/血钾异常基因检测，包含44个疾病相关基因，筛查15种疾病亚型。";
	}
	if($types eq "WES")
	{
...
END
	...
	if($types eq "GXY")
	{
		$part1_list.="{\\sym{附表$fb_num：}}";
		$part1_list.=<<'END';
本检测共筛查15种单基因高血压/血钾异常疾病，包含44个相关基因，具体疾病名称请参见下表。 \\
\renewcommand\arraystretch{1.8}
\fontsize{10.5}{12.6}\selectfont
...
END
		...
		open IN, "/PUBLIC/pipline/script/Report/XKFW/V3_180824/database/gxy_disease_name3.txt" or die $!;
		while(<IN>)
		{
			chomp;
			@cut=split /\t/,$_;
...
END
		...
		while(<IN>)
		{
			...
			if($cut[0] eq $samples )
			{
				...
				if ($cut[1] eq "高血压") {
					$cut[1] = "单基因高血压";
					$part1_list.="\\makecell[{{p{4cm}}}]{单基因高血压} & \\makecell[{{p{11cm}}}]{AIP, ARMC5, BSND, CACNA1D, CACNA1H, CASR, CDKN1B, CLCN2, CLCNKA, CLCNKB, CUL3, CYP17A1, EGLN1, EGLN2...} \\\\ \\hline \n";
					next;
				}
...
\zihao{6} \color{MyFontGray}
{\sym{说明：}} \\
1. {\sym{遗传模式：}}疾病对应的遗传模式，分为 AD（常染色体显性遗传）、AR（常染色体隐性遗传）、XL(X 染色体连锁)、XD/XLD（X 染色体显性遗传）、 XR/XLR（X 染色体隐性遗传）。常染色体隐性遗传病杂合突变一般不会表现出临床症状或症状轻微，X连锁隐性遗传病，女性杂合变异一般不会表现出临床症状或症状轻微。\\
2. {\sym{核苷酸变异：}}\\
...
\hspace*{3mm}   1) {\sym{错义变异：}}碱基的变异导致其所在氨基酸的改变。如：p.氨基酸（数字）氨基酸，如 p.Met129Ile，表示：该基因所编码的第 129 个氨基酸由Met(蛋氨酸)变为
 Ile(异亮氨酸)；\\
...
\hspace*{3mm}   4) {\sym{移码变异：}}指由于插入或缺失 N 个（非 3 整数倍）碱基的变异，这类变异会导致读码框发生错位，从而导致该位置及之后的氨基酸序列的明显改变。表示为：
p.氨基酸（数字1）氨基酸 fs * (数字2)，fs 表示 frameshift（移码）。数字 2 表示从突变位置开始算起，后面还可以翻译但框移的氨基酸的数量。如p.Arg311LysfsTer7，即表示：从第 311 号氨基酸 Arg 开始的氨基酸合成发生改变，并在改变后的第 7 个氨基酸终止; \\
{\sym{4、致病性等级}} \\
...
{\sym\sye{5、ACMG证据条目}} \\
\hspace*{3mm} {\sye{PVS:}} 非常强致病证据; {\sye{PS:}} 强致病证据; {\sye{PM:}} 中等致病证据; {\sye{PP:}} 支持性致病证据; {\sye{BP:}} 支持性良性证据; {\sye{BS:}} 强良性证据; {\sye{BA:}} 独立良性证据    \\
\end{spacing}
END
...
if($types eq "GXY")
{
	$part7_1="1、本次检测重点关注疾病数据库收录的与单基因高血压/血钾异常病相关的44个基因的全部外显子区域";
}
if($types eq "WES")
{
...
3、随着研究的深入和相关报道的增加，对部分变异的归类和解释可能发生变化，我们将会阶段性更新，本次检测使用的Clinvar数据库为2019年10月版。
\end{spacing}
END
...
if($negative == 0)
{
	$all_tex=$yemei.$biaoti2.$info_sample.$part1.$part2."\n \\end{longtable}\n\\fontsize{10.5}{12.6}\\selectfont\n".$part3.$BZ3."\\fontsize{10.5}{12.6}\\selectfont \n%\\noindent \\\\ \n\\begin{spacing}{1.3}\n{\\sym {检测结果说明：}} \\\\ \n".$part4."\\end{spacing}\n".$part5.$part6_0.$part1_list.$part6.$part7.$part8.$check."\n\\end{document} \n";
}
elsif($negative == 1)
{
	$all_tex=$yemei.$biaoti2.$info_sample.$part1.$part6_0.$BZ3.$part1_list.$part6.$part7.$part8.$check."\n\\end{document} \n";
}
elsif($negative == 2)
{
