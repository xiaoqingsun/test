...
#	print "$sample_report_database,$uptime_sa\n$all_report_database,$uptime_saAll\n$readdatabase,$uptime\n$readtrace,$uptimetrace\n";
	...
	if($type eq 'GXY'){
		...
		%sample_data=readGXY_sample_report_data($sample_report_database,\%all_disease,$uptime_sa);
        	%database=&updata_read_base($readdatabase,$gxy_sitedb_path,84,'XK_gxybase',\%sample_data,$uptime_sa);
	        &updata_read_base_fubiao(\%database,$gxy_sitedb_path_fubiao,'XK_gxybase_fubiao',\%sample_data,$uptime_sa,$readfubao);
		...
		%database=&updata_read_base($readdatabase,$cusi_sitedb_path,85,'XK_cusbase',\%sample_data,$uptime_sa);
        	&updata_read_base_fubiao(\%database,$cusi_sitedb_path_fubiao,'XK_CSbase_fubiao',\%sample_data,$uptime_sa,$readfubao);
		...
		&updata_all_report_database($all_report_database,$cusi_report_all_path,'CuS_reportAllbase',\%sample_data,$uptime_sa);
	        ...
	        %sample_data=&readCS_sample_report_data($sample_report_database,\%all_disease,$uptime_sa,$type);
        	%database=&updata_read_base($readdatabase,$dm_sitedb_path,85,'XK_dmbase',\%sample_data,$uptime_sa);
	        &updata_read_base_fubiao(\%database,$dm_sitedb_path_fubiao,'XK_DMbase_fubiao',\%sample_data,$uptime_sa,$readfubao);
		...
		%sample_data=&readCS_sample_report_data($sample_report_database,\%all_disease,$uptime_sa,$type);
		%database=&updata_read_base($readdatabase,$wes_sitedb_path,85,'XK_wesbase',\%sample_data,$uptime_sa);
		&updata_read_base_fubiao(\%database,$wes_sitedb_path_fubiao,'XK_WESbase_fubiao',\%sample_data,$uptime_sa,$readfubao);
...
sub updata_read_base_fubiao{
	my ($hash,$baspath,$basn,$sample_datas,$uptime_saa,$readfubaoo)=@_;
	make_path("$baspath/$uptime_saa");
        my $basefilename="$baspath/$uptime_saa/$basn$uptime_saa.right.txt";
        open(OUT,">$basefilename")||die;
        my %hash4;
		...
		while(<IN>){
			chomp;
			my @tmp=split /\t/,$_;
                	my $key2=join("_",@tmp[3..7,56]);
                	if(!exists $$hash{$key2} || $_=~/Flag/){
                        	$hash4{$key2}=1;
				print OUT  "$_\n";
			}
		}
	...
	foreach my $key(keys %{$$sample_datas{fubiao}}){
		if(!exists $$hash{$key}  && !exists $hash4{$key}){
...
sub updata_read_base{
	my ($basefile,$baspath,$discol,$basn,$sample_datas,$uptime_saa)=@_;
	make_path("$baspath/$uptime_saa");
	my $basefilename="$baspath/$uptime_saa/$basn$uptime_saa.right.txt";
	my %hash2;
        my %hash3;
	open(OUT,">$basefilename")||die;  
		...
		while(<IN>){
			chomp;
			my @tmp=split /\t/,$_;
			my $key=join("_",@tmp[3..7,56,$discol]);
                	my $key2=join("_",@tmp[3..7,56]);
			$hash2{$key}=1;
                	$hash3{$key2}=1;
...
sub readGXY_sample_report_data{
	...
	while(<IN>){
		chomp;
                ...
                my ($key,$keysam);
                $key=join("_",@tmp[3..7,56,84]);
                $keysam=join("_",@tmp[2..7,56,84]);
                $hash{samsite}{$tmp[2]}.="$_\t$uptime_saa\n";  
                if($tmp[0] =~/report/i){ 
                        my $person=$head{'解读人'};
                        $hash{site}{$key}=join("\t",$tmp[$person],@tmp[1..118]);
                        $tmp[0]=$uptime_saa;
                        $hash{reportsite}{$keysam}=join("\t",@tmp);
                }elsif($tmp[0] =~/fubiao/i){ 
                        my $key4=join("_",@tmp[3..7,56]);
...
sub readCS_sample_report_data{
	...
	while(<IN>){
		...
		die "请检查$gene_disea 是否和数据库一致 " if($typee ne 'WES'&& $tmp[1] !~/product/i  && !exists $$all_diseases{$gene_disea} && $tmp[0] !~/Negative/i);
                my ($key,$keysam);
		$key=join("_",@tmp[3..7,56,85]);
                $keysam=join("_",@tmp[2..7,56,85]);
		$hash{samsite}{$tmp[2]}.="$_\t$uptime_saa\n";
		if($tmp[0] =~/report/i){
			my $person=$head{'解读人'};
