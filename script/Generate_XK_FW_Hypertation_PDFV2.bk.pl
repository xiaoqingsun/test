...
#       say "订单${sample_id}中${key}位点未在高血压位点检出率文件（路径为：${control_detection_path}）中存在，请核查${sample_id}样本是否执行过mergevcf.sh程序。如未执行，请执行此程序；如有执行，请核查高血压config文件（路径：${hypers_cfg_addsex_path}）最新的文件中是否存在此订单，而高血压检出率文件中未找到此订单。如有此种情况，请核查mergevcf.sh的正确，并在高血压config文件去掉此订单，并重新运行mergevcf.sh程序" if ($case_ditacion_term eq "-\t-\t-\t-");
	...
	for(my $i=62;$i<66;$i++)
	{
		if($cut[$i] eq "-"){ $flag_dm_null++; next; }
		@flag_dm_array=split /\|/,$cut[$i];
		for(@flag_dm_array)
		{
			if( $i==62 or $i==64 or $i==65 ){
				$flag_dm=(split /:/,$_)[3];
			}
			elsif($i==63 ){
				$flag_dm=(split /:/,$_)[4];
			...
			if($flag_dm=~/DM|pathogenic|significance/i && $FLAG_DM==0)
			{
				$FLAG_DM=1;
			}
		}
	}
	if($flag_dm_null>=3){$FLAG_DM=1;}
	if($FLAG_DM==0){push @filter_type, "non_pathogenic_DB";}

	$hgmd_flag=1 if( $cut[43]=~/true/i && $cut[43]=~/DM/i );
	
	my $flag=(@filter_type) ? (join",",@filter_type) : '?';
	if (($cut[52] ne "-" and $flag!~/Man_pregnancy_hypertension/i ) or ($hgmd_flag==1 && $ratio1000>2 && $Disease_Namein_flag==0)  or @filter_type<1){  ### panqi 181017 add ratio1000 判断
		++$sample_number{$sample_id};
...
sub add_sitebase_info{
	...
	if($cut[30]=~/het/ and $add_info[7]=~/AR/){$add_info[4]="由于该疾病为隐性遗传病，本次检出杂合变异，一般情况下不会出现疾病的相应症状或者症状轻微，".$add_info[4];}#wph add 190104
        ...
        if($ncon==4) {
              ...
              if($ratio_chinese ne '未收录'){$ratio_detection.="千人项目中国人群检出频率为$ratio_chinese，";}
	...
	if($cut[5]=~/splicing/){
		...
		$muty="会导致$cut[14]${mutation_type}变异";
	}
	$muty=~s/^-//;
