...
{\sym{备注：}} \\
{\sym{1、致病性等级}} \\
1） 致病： 根据 ACMG 基因变异解读标准，该位点突变有多条致病类证据，是致病的重要原因。\\
2） 可能致病： 根据 ACMG 基因变异解读标准，该位点突变有一定数量的致病类证据，其导致疾病的可能性大于 90\%。\\
3） 临床意义未明 1 级： 根据 ACMG 基因变异解读标准，该位点突变有一条非常强的致病类证据，但总体证据条目不足，因此其致病性无法准确判定，需结合临床信息综合判断。\\
4） 临床意义未明 2 级： 根据 ACMG 基因变异解读标准，该位点突变有一条中等的致病类证据，但总体证据条目不足，因此其致病性无法准确判定，需结合临床信息综合判断。\\
5） 临床意义未明 3 级： 根据 ACMG 基因变异解读标准，该位点突变有一条较弱的致病类证据，但总体证据条目不足，因此其致病性无法准确判定，需结合临床信息综合判断。\\
6） 临床意义未明 4 级： 根据 ACMG 基因变异解读标准，该位点突变找不到任何证据条目，或证据存在矛盾情况，因此其致病性无法准确判定，需结合临床信息综合判断。\\
7） 临床意义未明 5 级： 根据 ACMG 基因变异解读标准，该位点突变有一条较弱的良性类证据，但总体证据条目不足，因此其致病性无法准确判定，需结合临床信息综合判断。\\
8） 可能良性： 根据 ACMG 基因变异解读标准，该位点突变有一定数量的良性类证据，其不会致病的可能性大于 90\%。\\
9） 良性： 根据 ACMG 基因变异解读标准，该位点突变有多条良性类证据，因此为良性变异，不会导致疾病发生。\\
{\sym{2、ACMG证据条目}} \\
1） PS: 强致病证据     \\
2） PM: 中等致病证据   \\
3） PP: 支持性致病证据 \\
...
\hline
{\sym{变异基因}} & {\sym{核苷酸变异}} & {\sym{氨基酸变异}} & \makecell[c]{ \sym{转录本}\\ \sym{外显子} } & {\sym{变异状态}} & {\sym{变异类型}} & {\sym{致病性}} & \makecell[c]{\sym{相关疾病(遗传模式)}} & \makecell[c]{\sym{ACMG证据条目}} \\ \hline
END
...
#$arr[$i][1]：$arr[$i][3] \\\\ \\\\ \n];
			...
			$part2_2=qq[ & $c7 & \\makecell[{}{m{34mm}}]{$c8} & \\makecell[{}{m{34mm}}]{$c9} \\\\ \\cline{7-9}\n ] if($j==0 && $max_row>1);
			$part2_2=qq[ & $c7 & \\makecell[{}{m{34mm}}]{$c8} & \\makecell[{}{m{34mm}}]{$c9} \\\\ \\hline\n ] if($j==0 && $max_row==1);
			$part2_2.=qq[ & & & & & & $c7 & \\makecell[{}{m{34mm}}]{$c8} & \\makecell[{}{m{34mm}}]{$c9} \\\\ \\cline{7-9}\n ] if($j>0 && $j+1!=$max_row);
			$part2_2.=qq[ & & & & & & $c7 & \\makecell[{}{m{34mm}}]{$c8} & \\makecell[{}{m{34mm}}]{$c9} \\\\ \\hline\n ] if($j>0 && $j+1==$max_row);
...
\zihao{6} \color{MyFontGray}
{\sym{备注：}} \\
1. 遗传模式：疾病对应的遗传模式，分为 AD（常染色体显性遗传）、AR（常染色体隐性遗传）、XL(X 染色体连锁)、XD/XLD（X 染色体显性遗传）、 XR/XLR（X 染色体隐性遗传）。\\
2. 核苷酸变异：\\
...
3. 氨基酸改变：核苷酸变异会导致其所编码的氨基酸产生变化，具体分为：\\
\hspace*{3mm}   1) 错义变异：碱基的变异导致其所在氨基酸的改变。如：p.氨基酸（数字）氨基酸，如 p.Met129Ile，表示：该基因所编码的第 129 个氨基酸由Met(蛋氨酸)变为 Ile(异亮氨酸)；\\
\hspace*{3mm}   2) 无义变异：指由于某碱基的变化导致其所在氨基酸变成终止密码子。表示为：p.氨基酸(数字) * 。如p.Gln116*，表示：该基因所编码的第116个氨基酸由 Gln（ 谷氨酰胺） 变为终止密码子；\\
\hspace*{3mm}   3) 剪接位点变异：这种变异会影响基因的转录过程，导致转录产物 mRNA 序列的异常，从而影响蛋白功能。这类变异不会直接导致氨基酸的变化，故而标记为 ` $-$ ' ， 一般位于剪切位点±1/2的变异，对剪切影响较大。\\
\hspace*{3mm}   4) 移码变异：指由于插入或缺失 N 个（非 3 整数倍）碱基的变异，这类变异会导致读码框发生错位，从而导致该位置及之后的氨基酸序列的明显改变。表示为：p.氨基酸（数字1）氨基酸 fs * (数字2)，fs 表示 frameshift（移码）。数字 2 表示从突变位置开始算起，后面还可以翻译但框移的氨基酸的数量。如p.Arg311LysfsTer7，即表示：从第 311 号氨基酸 Arg 开始的氨基酸合成发生改变，并在改变后的第 7 个氨基酸终止; \\
END
...
\zihao{6} \color{MyFontGray}
{\sym{备注：}} \\
1、拷贝数列 {\sye{1}} 意味着该片段在该患者样本中存在1个拷贝，可能存在片段缺失情况， {\sye{3}} 意味着该片段在患者样本中存在3个拷贝，可能存在片段重复的情况， {\sye{0}} 意味着2个 拷贝全部缺失。\\
...
if($negative == 0)
{
	$all_tex=$yemei.$biaoti2.$info_sample.$part1.$part2."\n \\end{longtable}\n".$part3."\\fontsize{10.5}{12.6}\\selectfont \n%\\noindent \\\\ \n\\begin{spacing}{1.3}\n{\\sym {检测结果说明：}} \\\\ \n".$part4."\\end{spacing}\n".$part5.$part6_0.$part7.$part1_list.$part6.$part8.$check."\n\\end{document} \n";
}
elsif($negative == 1)
{
	$all_tex=$yemei.$biaoti2.$info_sample.$part1.$part6_0.$part7.$part1_list.$part6.$part8.$check."\n\\end{document} \n";
}
elsif($negative == 2)
{
	$all_tex=$yemei.$biaoti2.$info_sample.$part1.$part7.$part1_list.$part6.$part8.$check."\n\\end{document} \n";
}
