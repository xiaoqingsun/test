...
END
	...
	if($reportflag=~/南方/){
		if($module=~m/GXY/i){
			...
			$script.="ln -s $outdir/02individual/$ID/$ID.单基因高血压基因检测Panel报告.docx $outdir/03result/南方医院-诺禾心康-$ID\_$name\_单基因高血压血钾异常基因检测报告.docx\n";	
		}
		if($module=~m/CS/i){
	...
	{   # wangpenghui add 20180531  #190314 wph 删除其他模块报告，只留全外
		...
		if($reportflag=~/南方/){
			...
			$script.="ln -s $outdir/02individual/$ID/$ID\.遗传病全外显子测序报告.docx $outdir/03result/南方医院-诺禾心康-$ID\_$name\_全外显子基因检测.docx\n";
		}
	}
